openerp_oa_notification = function (instance) {
     ...
     */
        ...
        init: function (parent, datasets, options) {
            ...
            if(this.parent_id){
            	...
            	this.options.show_reply_button=false;
            }
        ...
        format_data: function () {
            ...
            this.display_date = this.date.toString('yyyy-MM-dd');
            ...
            if (this.author_avatar) {
                ...
                this.avatar = mail.ChatterUtils.get_image(this.session, 'res.partner', 'image', this.author_id[0]);
            } else {
    ...
    mail.ThreadExpandable = mail.MessageCommon.extend({
        ...
        init: function (parent, datasets, options) {
            ...
            this.nb_unread=datasets.nb_unread;
        ...
        start: function () {
            this._super.apply(this, arguments);
            this.bind_events();
            this.$el.parent().prev().find(".nb_unread_right").text("(未读:"+this.$el.find(".nb_unread").text()+"条)");
        },
...
//            this.flag_used = true;
            ...
            self.parent_thread.message_fetch(this.domain, this.context, false, function (arg, data) {
                ...
                if(element.find(".message_hide").val().indexOf("显示")>=0){
	                self.parent_thread.switch_new_message(data, element);
	                //self.animated_destroy(200);
	                
	                element.find(".message_hide").val("隐藏消息");
	                //self.parent_thread.$el.hide();
	                //同步右上角未读条数
	                element.parent().prev().find(".nb_unread_right").text("(未读:0条)");
	                //更改图标
	                element.parent().prev().find("i:first").attr("class","icon-double-angle-up icon-large");
                }
                else{
                	element.nextAll().hide();
                	element.find(".message_hide").val("显示消息");
                	//同步右上角未读条数
	                element.parent().prev().find(".nb_unread_right").text("(未读:0条)");
	                //更改图标
	                element.parent().prev().find("i:first").attr("class","icon-double-angle-down icon-large");
                }
    ...
    mail.ThreadMessage = mail.MessageCommon.extend({
        ...
        start: function () {
            ...
            //this.$el.find(".oe_star").hide();
            //this.$(".nb_unread_right").text();
        },
        ...
        bind_events: function () {
            ...
            this.$('.oe_expand_right').on('click', this.on_message_expand);
        ...
        on_message_expand:function(){
        	var self=this;
        	var element=self.$el.next().find(".oe_msg_more_message");
    ...
    mail.RecordThread = session.web.form.AbstractField.extend({
        ...
        render_value: function () {
            ...
            this.node.context = {
            	//liuhongtai 2015-12-11 点击消息后是否设置已读
                'mail_read_set_read': false,  // set messages as read in Chatter
    ...
    mail.Wall = session.web.Widget.extend({
        ...
        start: function () {
            ...
            //控制页面最小宽度
            $(".oe_view_manager_body").css("min-width","960px");
            //取当前用户staff_id以及各提醒条数
            new openerp.Model("res.users").query(["id","partner_id"]).filter([["id","=",this.session.uid]])
        	.first().then(function(pic_name){
        		//查询提醒数目
        		new openerp.Model("oa.attendance").call('get_notification_num',[],{context: new instance.web.CompoundContext()})
            	.then(function (result) {
            		//今天日期
            		var today=new Date();
            		var dayNames = new Array("星期日","星期一","星期二","星期三","星期四","星期五","星期六");  
            		var today_info="今天是" + today.getFullYear() + "年" + (today.getMonth()+1) + "月" + today.getDate() + "日  " + dayNames[today.getDay()];
            		if(result["oa.task"]==undefined||result["oa.task"]==""){
            			result["oa.task"]="0";
            		}
            		if(result["oa.assess"]==undefined||result["oa.assess"]==""){
            			result["oa.assess"]="0";
            		}
            		if(result["oa.holidays.trip"]==undefined||result["oa.holidays.trip"]==""){
            			result["oa.holidays.trip"]="0";
            		}
            		// render sidebar
            		var sideBar = $(QWeb.render("mail.wall.sidebar",{today_info:today_info,pic_name:pic_name,result:result}));
            		self.$el.find(".oe_mail_wall_aside").append(sideBar);
            	});
        	});
