...
class Root(object):
    ...
    def setup_session(self, httprequest):
        #print "handle request: " + str(httprequest)
        ...
        if not sid:
            sid = httprequest.form.get('session_id')
        if sid is None:
            httprequest.session = self.session_store.new()
#             httprequest.headers.setdefault('X-Openerp-Session-Id', httprequest.session.sid)
#             print "new session, sid: " + httprequest.session.sid + ", session: " + str(httprequest.session)
        else:
            httprequest.session = self.session_store.get(sid)
            #print "get session from file: " + sid + ", session:" + str(httprequest.session)
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
			...
			#modified by wangkui, for mobile app session management
            try:
                if request.params:
                    if request.params.get('params'):
                        params = request.params['params']
                        params_list = json.loads(params)
                        if params_list.get('filter'):
                            if params_list.get('filter').get('db'):
                                request.session.db = params_list.get('filter').get('db')
            except werkzeug.exceptions.HTTPException, e:
