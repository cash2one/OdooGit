...
def clean_action(action):
    ...
    # added by duzhigao, set the way of displaying header part of view in context attribute of action
    dict_context = {}
    if(action.get('context')):
        if(isinstance(action.get('context'), dict)):
            dict_context = action['context']
        else:
            try:
                dict_context = eval(action['context'])
            except:
                dict_context = {}
    c_views_swither = dict_context.get('views_switcher')
    c_search_view = dict_context.get('search_view')
    c_action_buttons = dict_context.get('action_buttons')
    c_sidebar = dict_context.get('sidebar')
    c_pager = dict_context.get('pager')
    c_display_title = dict_context.get('display_title')
    c_search_disable_custom_filters = dict_context.get('search_disable_custom_filters')
    
    #dict_flags = eval(action['flags'])
    if ('views_switcher' in dict_context) or ('search_view' in dict_context) or ('action_buttons' in dict_context) or ('sidebar' in dict_context) or ('pager' in dict_context) or ('display_title' in dict_context) or ('search_disable_custom_filters' in dict_context):
        action['flags'] = {'views_switcher': False, 'search_view': False,  'action_buttons': False, 'sidebar': False, 'pager': False, 'display_title': False, 'search_disable_custom_filters': False}
    if c_views_swither:
        action['flags']['views_switcher'] = c_views_swither
    if c_search_view:
        action['flags']['search_view'] = c_search_view
    if c_action_buttons:
        action['flags']['action_buttons'] = c_action_buttons
    if c_sidebar:
        action['flags']['sidebar'] = c_sidebar
    if c_pager:
        action['flags']['pager'] = c_pager
    if c_display_title:
        action['flags']['display_title'] = c_display_title
    if c_search_disable_custom_filters:
        action['flags']['search_disable_custom_filters'] = c_search_disable_custom_filters
...
class Home(http.Controller):
    ...
    @http.route('/web/attachment/download', type='http', auth="user")
    def download_attachment(self, attachment_id, **kw):
        
        ensure_db()
        #下载单个文件
        Model = request.registry.get('ir.attachment')
        #Model = request.env['ir.attachment']
        try:
            res = Model.get_attachment(request.cr, request.uid, int(attachment_id))
            if res:
    ...
    @http.route('/web/attachment/upload', type='http', auth="user")
    @serialize_exception
    def upload_attachment(self, Filedata,Upload, Filename,callback,model,id):
        ...
        try:
            attachment_id = Model.create({
                ...
                'datas': base64.encodestring(Filedata.read()),
                'datas_fname': Filedata.filename,
                'res_model': model,
                'res_id': int(id)
            }, request.context)
            args = {
...
class Binary(http.Controller):
    ...
    def upload_attachment(self, callback, model, id, ufile):
        ...
        if(callback == 'get_attachment_id'):
            out = '{"id":%s,"filename":"%s"}'
            return out % (args['id'],args['filename'])
        else:
            return out % (simplejson.dumps(callback), simplejson.dumps(args))
