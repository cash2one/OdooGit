...
instance.web.Dialog = instance.web.Widget.extend({
    ...
    */
    init: function (parent, options, content) {
        ...
        if(options.footer == false){
        	this.dialog_options.footer = false;
        }else{
        	this.dialog_options.footer = true;
        }
        if(this.dialog_options.footer){
    ...
    */
    init_dialog: function() {
        ...
        if (options.buttons && this.dialog_options.footer) {
...
instance.web.Menu =  instance.web.Widget.extend({
    ...
    bind_menu: function() {
        ...
        instance.web.bus.on('resize', this, function() {
            if (parseInt(self.$el.parent().css('width')) <= 1024 ) {
                lazyreflow();
            } else {
                lazyreflow();
            }
    ...
    open_menu: function (id) {
        ...
        if ($main_menu !== $clicked_menu) {
            ...
            if ($clicked_menu.is('.oe_menu_toggler')) {
            	//if($clicked_menu.parent().parent().css('display') != 'none' && !$clicked_menu.parent().parent().siblings().children('i').hasClass('next_level')){
            		//$clicked_menu.parent().parent().siblings().children('i').addClass('next_level');
            	//}else{
            	   $clicked_menu.children('i').toggleClass('next_level');
            	//}
                $clicked_menu.toggleClass('oe_menu_opened').siblings('.oe_secondary_submenu:first').toggle();
            } else {
                $clicked_menu.parent().addClass('active');
                //liuhongtai 2015-12-04 add control menu problem
                var up_menu=$clicked_menu.parent().parent().parent().find("a:first");
                if(up_menu.hasClass('oe_menu_toggler')){
                	up_menu.find("i:first").addClass('next_level');
                }
            }
        }
    ...
    do_update: function () {
        ...
        var fct = function() {
            ...
            return self.alive(func(self.session.uid, ["name", "company_id"])).then(function(res) {
                ...
                var avatar_src = self.session.url('/web/binary/image', {model:'res.users', field: 'image', id: self.session.uid});
...
instance.web.WebClient = instance.web.Client.extend({
    init: function(parent, client_options) {
        ...
        this.set('title_part', {"zopenerp": "HSE工作平台"});
    },
    start: function() {
        var self = this;
