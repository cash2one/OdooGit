        ...
        case $.ui.keyCode.BACKSPACE:
            ...
            if (sel.start === 0 && sel.start === sel.end) {
                ...
                if (preceding && (preceding instanceof my.FacetView)) {
                    ...
                    if($.trim(preceding.$('.oe_facet_category').text()).indexOf('Advanced')>=0 || $.trim(preceding.$('.oe_facet_category').text()).indexOf('高级选项')>=0){
                        $('button.oe_apply').prop('disabled', true);
                        $('.oe_searchview_advanced.oe_opened ul li').remove();
                    }
                }
            }
            break;
...
instance.web.SearchView = instance.web.Widget.extend(/** @lends instance.web.SearchView# */{
    ...
    events: {
        ...
        'click .oe_searchview_clear': function (e) {
            ...
            $('button.oe_apply').prop('disabled', true);
            $('.oe_searchview_advanced.oe_opened ul li').remove();
            ///jQuery('.oe_searchview_advanced ul').children().remove();
        },
    ...
    start: function() {
        ...
        if (this.headless) {
            ...
            this.alive($.when(load_view)).then(function (r) {
                ...
                if(r.arch.attrs.expand_search && r.arch.attrs.expand_search=='true'){
                	self.options.expand_search=true;
                }else{
                	self.options.expand_search=false;
                }
                if(r.arch.attrs.fix_fields){
                    self.drawer.fix_fields=r.arch.attrs.fix_fields.split(',');
                }
        ...
        if (view_manager) {
            ...
            view_manager.on('switch_mode', this, function (e) {
            	if(self.options.expand_search){
            		self.drawer.toggle(e === 'graph' || self.options.expand_search);
            	}else{
...
function remove_advance_prop(){
	jQuery('.oe_searchview_advanced ul').children().remove();
	//openerp.PropertiesMixin.destroy.call(obj);	
}
instance.web.search.Advanced = instance.web.search.Input.extend({
    ...
    start: function () {
        ...
        this.$el
            ...
            .on('click', 'h4', function () {
                self.$el.toggleClass('oe_opened',function(){
        ...
        return $.when(
            ...
            new instance.web.Model(this.view.model).call('fields_get', {
                    ...
                    self.fields = {
                        id: { string: 'ID', type: 'id', searchable: false }
                    };
                    _.each(data, function(field_def, field_name) {
                        if (field_def.selectable !== false && field_name != 'id' && field_def.searchable == true) {
                            self.fields[field_name] = field_def;                            
                        }
                    });
        })).done(function () {
        	var fixed_fields=self.drawer.fix_fields;
        	var count=0;
        	if(fixed_fields && fixed_fields.length>0){        		
        		for(var i=0;i<fixed_fields.length;i++){        			
        			if(self.fields[self.drawer.fix_fields[i]]){
        				var obj={};
        				obj[self.drawer.fix_fields[i]]=self.fields[self.drawer.fix_fields[i]];
        				self.append_fix_proposition(obj);
        				count+=1;
        			}
        		}
        		if(count==0){
        			self.append_proposition();
        			self.$('button.oe_add_condition').show();
        		}else{
        			self.$('button.oe_add_condition').hide();
        			self.$('h4').click();
        		}
        	}else{
        		self.append_proposition();
    ...
    append_fix_proposition: function (field) {
        var self = this;
        return (new instance.web.search.ExtendedSearchProposition(this, field))
            .appendTo(this.$('ul')).done(function () {
                self.$('button.oe_apply').prop('disabled', false);
            });
    },
    ...
    commit_search: function () {
        ...
        var operates = this.$el.find('.searchview_extended_prop_or')
        if(children.length != operates.length){
        	var c_count=children.length-operates.length;
        ...
        //for (var i = domain.length; --i;) {
            //domain.unshift('|');
        //}
        for(var i = 1;i<operates.length;i++){
        if(jQuery(operates[i]).val() == 'and' || jQuery(operates[i]).val() == '且'){
        domain.unshift('&');
        }
        else{
        domain.unshift('|');
