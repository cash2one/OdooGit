...
instance.web.ListView.List = instance.web.Class.extend( /** @lends instance.web.ListView.List# */{
    ...
    render_cell: function (record, column) {
        ...
        if(column.type === 'reference') {
        	...
        	attach_value = ir_attachment.call('get_attachments', {
    					ids : [],
         ...
         .done(function(r) {
    					if(r && r.length > 0){
    				        return _.template('<a href="<%-href%>"><%-text%></a>', {
    				            text: r[0].name,
    				            href: '/web/attachment/download?attachment_id='+r[0].id,
    				            size:'12MB'
    				        });
    					}else{
    ...
    /*modify by liuhongtai 20151102 add function render_cell_sub,used in base.xml*/
    render_cell_sub: function (record, column) {
        var value;
        if(column.type === 'reference') {
            value = record.get(column.id);
            var ref_match;
        ...
        var cell_content=column.format(record.toForm().data, {
            model: this.dataset.model,
            id: record.get('id')
        });
        if(cell_content){
            var cell_len=cell_content.length;
            //&& <input 表示单选框   data:application 表示附件
            if(this.dataset.model!="pm.impl.organ.plan.content" && this.dataset.model!="pm.impl.plan.baseline.version" && this.dataset.model!="pm.impl.plan.baseline"
            	 && this.dataset.model!="pm.purchase.zb.bd"){
    	        if(cell_content.length>25 && cell_content.indexOf("<input")==-1 && cell_content.indexOf("data:application")==-1){
    	        	cell_content=cell_content.substr(0,25)+"...";
    	        }
            }
        }else{
        	cell_content = '';
    ...
    render_record: function (record) {
        ...
        return QWeb.render('ListView.row', {
            ...
            render_cell: function () {
                return self.render_cell.apply(self, arguments); },
            /*modify by liuhongtai 20151102 add*/
        	render_cell_sub: function () {
                    return self.render_cell_sub.apply(self, arguments); }
        });
    }
});
    ...
    render: function (post_render) {
        ...
        this.datagroup.list(
            ...
            function (groups) {
                ...
                self.view.$pager.find('.oe_list_pager_state').text(self.view._limit ? self.view._limit : '鈭�');
                $el[0].appendChild(
...
instance.web.list.Binary = instance.web.list.Column.extend({
    ...
    _format: function (row_data, options) {
		...
		// /web/attachment/download?attachment_id=

		/*return;
        var text = _t("Downloadgg");
