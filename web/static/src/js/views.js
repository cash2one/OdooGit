        ...
        return $.when(view_promise).done(function () {
            _.each(_.keys(self.views), function(view_name) {
                ...
                if (view_name === view_type) {
                	container.removeClass('mix_kanban_form');
                    container.show();
...
instance.web.ViewManagerAction = instance.web.ViewManager.extend({
    ...
    set_title: function(title) {
        ...
        // added by duzhigao, for special-sign page, change the title of the page.
        if(this.dataset.model == "oa.attendance") {
        	this.$el.find('.oe_breadcrumb_item:first').css("vertical-align", "middle");
        	this.$el.find('.oe_breadcrumb_item:first').html(this.action.name);
        }
    },
    do_push_state: function(state) {
        if (this.getParent() && this.getParent().do_push_state) {
