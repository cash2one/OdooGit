(function() {
	...
	openerp.sidebar_switch = true
	/** @namespace */
	instance.web.form = {};

	/**
	 * Interface implemented by the form view or any other object able to
	 * provide the features necessary for the fields to work.
	 * 
	 * Properties: - display_invalid_fields : if true, all fields where
	 * is_valid() return true should be displayed as invalid. - actual_mode :
	 * the current mode of the field manager. Can be "view", "edit" or "create".
	 * Events: - view_content_has_changed : when the values of the fields have
	 * changed. When this event is triggered all fields should reprocess their
	 * modifiers. - field_changed:<field_name> : when the value of a field
	 * change, an event is triggered named "field_changed:<field_name>" with
	 * <field_name> replaced by the name of the field. This event is not related
	 * to the on_change mechanism of OpenERP and is always called when the value
	 * of a field is setted or changed. This event is only triggered when the
	 * value of the field is syntactically valid, but it can be triggered when
	 * the value is sematically invalid (ie, when a required field is false). It
	 * is possible that an event about a precise field is never triggered even
	 * if that field exists in the view, in that case the value of the field is
	 * assumed to be false.
	 */
	instance.web.form.FieldManagerMixin = {
		...
		/**
		 * Returns the current value of a field present in the view. See the
		 * get_value() method method in FieldInterface for further information.
		 */
		get_field_value : function(field_name) {
		},
		/**
		 * Gives new values for the fields contained in the view. The new values
		 * could not be setted right after the call to this method. Setting new
		 * values can trigger on_changes.
		 * 
		 * @param {Object}
		 *            values A dictonary with key = field name and value = new
		 *            value.
		 ...
		 * 
		 * @param {Array}
		 *            expression An OpenERP domain.
		 * @return {boolean} The computed value of the domain.
		 */
		compute_domain : function(expression) {
		},
		/**
		 * Builds an evaluation context for the resolution of the fields'
		 * contexts. Please note the field are only supposed to use this context
		 * to evualuate their own, they should not extend it.
		 * 
	...
	/**
	 * Properties: - actual_mode: always "view", "edit" or "create". Read-only
	 * property. Determines the mode used by the view.
	 */
	instance.web.FormView = instance.web.View.extend(
			instance.web.form.FieldManagerMixin, {
				/**
				 * Indicates that this view is not searchable, and thus that no
				 * search view should be displayed (if there is one active).
				 */
				searchable : false,
				template : "FormView",
				 ...
				 * 
				 * @param {instance.web.Session}
				 *            session the current openerp session
				 * @param {instance.web.DataSet}
				 *            dataset the dataset this view will work with
				 * @param {String}
				 *            view_id the identifier of the OpenERP view object
				 * @param {Object}
				 *            options - resize_textareas :
				 *            [true|false|max_height]
				 * 
				 * @property {instance.web.Registry}
				 *           registry=instance.web.form.widgets widgets registry
				 *           for this form view instance
				 */
				init : function(parent, dataset, view_id, options) {
					...
					this.has_been_loaded.done(function() {
								...
								self.on("change:actual_mode", self,
					...
					instance.web.bus.on('clear_uncommitted_changes', this,
							function(e) {
								if (!this.can_be_discarded()) {
									e.preventDefault();
								}
							});
					if (options.action != null && options.action.target != null
							&& options.action.target == 'current_edit') {
				...
				load_form : function(data) {
					...
					this.rendering_engine
							.set_fields_registry(this.fields_registry);
					this.rendering_engine.set_tags_registry(this.tags_registry);
					this.rendering_engine
							.set_widgets_registry(this.widgets_registry);
					this.rendering_engine.set_fields_view(data);
					var $dest = this.$el.hasClass("oe_form_container")
							? this.$el
							: this.$el.find('.oe_form_container');
					...
					if (this.model != 'oa.task') {
						if (this.options.$buttons) {
							this.$buttons.appendTo(this.options.$buttons);
						} else {
							this.$el.find('.oe_form_buttons')
									.replaceWith(this.$buttons);
						}
					}
					this.$buttons.on('click', '.oe_form_button_create', this
									.guard_active(this.on_button_create));
					this.$buttons.on('click', '.oe_form_button_edit', this
									.guard_active(this.on_button_edit));
					this.$buttons.on('click', '.oe_form_button_save', this
									.guard_active(this.on_button_save));
					this.$buttons.on('click', '.oe_form_button_cancel', this
									.guard_active(this.on_button_cancel));
					if (this.options.footer_to_buttons) {
						this.$el.find('footer').appendTo(this.$buttons);
					}

					this.$sidebar = this.options.$sidebar
							|| this.$el.find('.oe_form_sidebar');
					if (!this.sidebar && this.options.$sidebar) {
						this.sidebar = new instance.web.Sidebar(this);
						if (instance.sidebar_switch && this.model != 'oa.task') {
							this.sidebar.appendTo(this.$sidebar);
						}
						if (this.fields_view.toolbar) {
							this.sidebar.add_toolbar(this.fields_view.toolbar);
						}
						if (self.fields_view.arch.attrs.delete != 'false') {
							this.sidebar.add_items('other', _.compact([self
											.is_action_enabled('delete')
											&& {
												label : _t('Delete'),
												callback : self.on_button_delete
											}]));
						}
						if (self.fields_view.arch.attrs.duplicate != 'false') {
							this.sidebar.add_items('other', _.compact([self
											.is_action_enabled('create')
											&& {
												label : _t('Duplicate'),
					...
					// Add bounce effect on button 'Edit' when click on readonly
					// page view.
					// .find(".oe_form_group_row,.oe_form_field,label,h1,.oe_title,.oe_notebook_page,
					// .oe_list_content").
					this.$el.on('click', function(e) {
								if (self.get("actual_mode") == "view") {
									var $button = self.options.$buttons
											.find(".oe_form_button_edit");
					...
					this.$el
							.find(".oe_form_field_status:not(.oe_form_status_clickable)")
							.on('click', function(e) {
								if ((self.get("actual_mode") == "view")) {
									var $button = self.$el
											.find(".oe_highlight:not(.oe_form_invisible)")
											.css({
														'float' : 'left',
														'clear' : 'none'
													});
					...
					/*
					 * this.$el.find('.oe_form_group_row').on('click',function(e){
					 * if(self.model == 'oa.task'){ if(self.get('actual_mode') ==
					 * 'view'){ //self.to_edit_mode(); } e.stopPropagation(); }
					 * });
					 */
					this.$el.find('.oe_form_group_row').on('keydown',
							function(e) {
								if (self.model == 'oa.task') {
				...
				widgetBlurred : function() {
					if (this.__clicked_inside) {
						// clicked in an other section of the form (than the
						// currently
				 ...
				 *            [options]
				 * @param {Boolean}
				 *            [mode=undefined] If specified, switch the form to
				 *            specified mode. Can be "edit" or "view".
				 * @param {Boolean}
				 *            [reload=true] whether the form should reload its
				 *            content on show, or use the currently loaded
				 *            record
				 * @return {$.Deferred}
				 */
				do_show : function(options) {
					var self = this;
					options = options || {};
					if (this.sidebar && this.model) {
						this.sidebar.$el.show();
					}
					if (this.$buttons && this.model) {
						this.$buttons.show();
					}
					this.$el.show().css({
								opacity : '0',
					...
					if (options.reload !== false) {
						shown = shown.then(function() {
									if (self.dataset.index === null) {
										// null index means we should start a
										// new record
										return self.on_button_new();
									}
									var fields = _
											.keys(self.fields_view.fields);
					...
					return shown.then(function() {
								self._actualize_mode(options.mode
										|| self.options.initial_mode);
								self.$el.css({
											opacity : '1',
				...
				save_on_fly : function(rec) {
					if (this.model == 'oa.task' && this.content_change) {
						var self = this;
						...
						self.save().done(function(result) {
							 ...
							 * self.content_change=false; self.to_view_mode();
							 * });
							 */
							if (!self.last_record) {
								self.last_record = self.datarecord;
							}
							self.content_change = false;
							if (self.ViewManager.views.kanban) {
								var group_id = self.last_record.task_state[0];
								var group;
								var kanban_groups = self.ViewManager.views.kanban.controller.groups;
								for (var i = 0; i < kanban_groups.length; i++) {
									if (kanban_groups[i].value == group_id) {
										group = kanban_groups[i];
										break;
									}
								}
								var record;
								if (group) {
									for (var i = 0; i < group.records.length; i++) {
										if (group.records[i].id == self.last_record.id) {
				...
				load_record : function(record) {
					...
					// added by hl . add checkbox tree in sys_audit module
					var sys_audit_ovaplan_model = 'audit.ovaplan.info'
					if (sys_audit_ovaplan_model == self.model){
						if($('#ltree_1').length > 0){
							loadTreeDataOfOvaForm('lhsh', self.get_selected_ids()[0]);
						}
						if($('#ltree_2').length > 0){
							loadTreeDataOfOvaForm('txsh', self.get_selected_ids()[0]);
						}
						if($('#ltree_3').length > 0){
							loadTreeDataOfOvaForm('zxsh', self.get_selected_ids()[0]);
						}
					}
					var sys_audit_plan_model = 'audit.plan.info'
					if (sys_audit_plan_model == self.model && self.get_selected_ids()[0] != null){
						if($('#lplantree').length>0){
							loadTreeDataOfPlanForm(self.get_selected_ids()[0]);
						}
					}
					// added by liuhongtai 2015-12-26 for jqgrid
					var model_list = ['pm.fund.proj.monthplan',
							'pm.fund.budget.version', 'pm.fund.account',
							'pm.init.proj.apply'];
					if (model_list.indexOf(self.model) > -1){
						if ($('#jqGrid').length > 0) {
							$.jgrid.gridUnload("jqGrid");
							createJqGrid(self.model, record, this);
						}
					}
					
					//调用函数控制编辑按钮和新建按钮的显隐
					if (self.model!="ir.module.module"){
						ShowOrHideButton(self.model,record,_t);
					}
					
					if (self.content_change) {
						self.save_on_fly(this.datarecord);
					}
					if (!record) {
						this.set({
									'title' : undefined
						...
						this
								.do_warn(
										_t("Form"),
										_t("The record could not be found in the database."),
										true);
						return $.Deferred().reject();
					}
					this.datarecord = record;
					this._actualize_mode();
					// modified by liuhongtai 2015-09-22: when title overlong
					// display 30 chars before
					if (record.id && record.display_name) {
						record.display_name = record.display_name.length > 15
								? record.display_name.substr(0, 15) + "..."
								: record.display_name;
					}
					this.set({
								'title' : record.id
					...
					_(this.fields).each(function(field, f) {
						...
						var result = field
								.set_value(self.datarecord[f] || false);
						field._inhibit_on_change_flag = false;
						set_values.push(result);
					});
					if (self.model!="ir.module.module"){
						control_form_delete_button(self.model,record,_t,self);
					}
					return $.when.apply(null, set_values).then(function() {
						...
						self.do_update_pager(record.id === null
								|| record.id === undefined);
						if (self.sidebar) {
							self.sidebar.do_attachement_update(self.dataset,
									self.datarecord.id);
						}
						if (record.id) {
							self.do_push_state({
						...
						self.$el.add(self.$buttons)
								.removeClass('oe_form_dirty');
						self.autofocus();
					});
				},
				/**
				 * Loads and sets up the default values for the model as the
				 * current record
				 * 
				 * @return {$.Deferred}
				 */
					...
					if (this.can_be_discarded()) {
						switch (action) {
							...
							case 'next' :
								...
								break;
							case 'last' :
								this.dataset.index = this.dataset.ids.length
										- 1;
								break;
						}
						var def = this.reload();
				...
				init_pager : function() {
					...
					if (this.options.$pager) {
						...
						this.$el.find('.oe_form_pager')
								.replaceWith(this.$pager);
					}
				...
				do_update_pager : function(hide_index) {
					...
					if (hide_index) {
						...
						$(".oe_form_pager_state", this.$pager).html(_.str
								.sprintf(_t("%d / %d"), this.dataset.index + 1,
										this.dataset.ids.length));
					}
				},
					...
					_.each(this.fields, function(field, name) {
						...
						_.each(field.field.views, function(view) {
							_.each(view.fields, function(_, subname) {
								self._onchange_specs[name + '.' + subname] = find(
										subname, view.arch);
							});
						});
					});
				...
				_get_onchange_values : function() {
					...
					if (field_values.id
							.toString()
							.match(instance.web.BufferedDataSet.virtual_id_regex)) {
						delete field_values.id;
					}
					if (this.dataset.parent_view) {
						// this belongs to a parent view: add parent field if
						// possible
						var parent_view = this.dataset.parent_view;
						var child_name = this.dataset.child_name;
						var parent_name = parent_view
								.get_field_desc(child_name).relation_field;
						if (parent_name) {
							// consider all fields except the inverse of the
							// parent field
					...
					// added by liuhongtai 2015-12-28 for jqGrid
					var model_list = ['pm.fund.budget.version',
							'pm.fund.account'];
					if (model_list.indexOf(this.model) > -1) {
						if (field_values.hasOwnProperty("proj_id")
								&& field_values.proj_id != false) {
							var _record = {};
							_record.proj_id = field_values.proj_id;
							// _record.flag = "OnChange";
							$.jgrid.gridUnload("jqGrid");
							createJqGrid(this.model, _record, this);
						}
					}
					
					//added by hl 0328 for checkbox tree
					if('audit.plan.info' == this.model){
						if(field_values.hasOwnProperty("audit_method") && field_values.audit_method != false
						&& field_values.hasOwnProperty("ovaplan_id") && field_values.ovaplan_id != false){
							loadInitTreeDataOfPlanForm(field_values.audit_method, field_values.ovaplan_id);
						}
					}
					return field_values;
				},
				...
				do_onchange : function(widget) {
					...
					try {
						var def = $.when({});
						var change_spec = widget
								? onchange_specs[widget.name]
								: null;
						if (!widget
								|| (!_.isEmpty(change_spec) && change_spec !== "0")) {
							var ids = [], trigger_field_name = widget
									? widget.name
									: self._onchange_fields, values = self
									._get_onchange_values(), context = new instance.web.CompoundContext(self.dataset
									.get_context());
							...
							if (self.dataset.parent_view) {
								var parent_name = self.dataset.parent_view
										.get_field_desc(self.dataset.child_name).relation_field;
								context.add({
							...
							if (self.datarecord.id
									&& !instance.web.BufferedDataSet.virtual_id_regex
											.test(self.datarecord.id)) {
								// In case of a o2m virtual id, we should pass
								// an empty ids list
								ids.push(self.datarecord.id);
							}
							def = self
									.alive(new instance.web.Model(self.dataset.model)
											.call("onchange", [ids, values,
															trigger_field_name,
															onchange_specs,
															context]));
						}
							...
							return def.then(function(response) {
								...
								_.each(fields, function(field, fieldname) {
									if (field && field.change_default) {
										var value_;
										if (response.value
												&& (fieldname in response.value)) {
											// Use value from onchange if
											// onchange executed
											value_ = response.value[fieldname];
										} else {
											// otherwise get form value for
											// field
											value_ = self.fields[fieldname]
													.get_value();
										}
										...
										if (value_) {
											defs
													.push(self
															.alive(new instance.web.Model('ir.values')
																	.call(
																			'get_defaults',
																			[
																					self.model,
								...
								return _.isEmpty(defs) ? response : $.when
										.apply(null, defs);
							}).then(function(response) {
										return self
												.on_processed_onchange(response);
									});
						});
				...
				on_processed_onchange : function(result) {
					try {
						...
						//added by hl 0328 for checkbox tree
						if('audit.plan.info' == this.model && result.value != null && result.value != undefined && result.value.hasOwnProperty("audit_method")){
							if (fields.hasOwnProperty("ovaplan_id") && fields.ovaplan_id.display_value != null && fields.ovaplan_id.display_value != undefined){
								var ovaplan_obj = fields.ovaplan_id.display_value;
								var ovaplan_id;
								for(var val in ovaplan_obj){
									ovaplan_id = parseInt(val);
								}
							}
							var audit_method = result.value.audit_method;
							loadInitTreeDataOfPlanForm(audit_method, ovaplan_id);
						}
					...
					return this.mutating_mutex.exec(function() {
						function iterate() {
							...
							return $.when.apply(null,
									[mutex.def, self.onchanges_mutex.def])
									.then(function() {
										var save_obj = self.save_list.pop();
										if (save_obj) {
											return self._process_save(save_obj)
													.then(function() {
														save_obj.ret = _
																.toArray(arguments);
														return iterate();
				...
				/**
				 * Ask the view to switch to view mode if possible. The view may
				 * not do it if the current record is not yet saved. It will
				 * then stay in create mode.
				 */
				to_view_mode : function() {
					this._actualize_mode("view");
				},
				/**
				 * Ask the view to switch to edit mode if possible. The view may
				 * not do it if the current record is not yet saved. It will
				 * then stay in create mode.
				 */
				...
				/**
				 * Ask the view to switch to a precise mode if possible. The
				 * view is free to not respect this command if the state of the
				 * dataset is not compatible with the new mode. For example, it
				 * is not possible to switch to edit mode if the current record
				 * is not yet saved in database.
				 * 
				 * @param {string}
				 *            [new_mode] Can be "edit", "view", "create" or
				 *            undefined. If undefined the view will test the
				 *            actual mode to check if it is still consistent
				...
				check_actual_mode : function(source, options) {
					...
					if (this.get("actual_mode") === "view") {
						self.$el.removeClass('oe_form_editable')
								.addClass('oe_form_readonly');
						self.$buttons.find('.oe_form_buttons_edit').hide();
						self.$buttons.find('.oe_form_buttons_view').show();
						self.$sidebar.show();
						//added by hl. 0405. change the checkbox tree status with form mode
						if('audit.ovaplan.info' == self.model || 'audit.plan.info' == self.model){
							setButtonStatus(true);
						}
					} else {
						self.$el.removeClass('oe_form_readonly')
								.addClass('oe_form_editable');
				...
				on_button_save : function(e) {
					...
					return this.save().done(function(result) {
						...
						self.reload().then(function() {
							self.to_view_mode();
							if (self.ViewManager.views.kanban) {
								self.ViewManager.views.kanban.controller
										.do_reload();
							}
							// self.ViewManager.views.kanban.controller.do_load_state(0);
				...
				on_button_cancel : function(event) {
					...
					if (this.can_be_discarded()) {
						if (this.get('actual_mode') === 'create') {
							...
							this.to_view_mode();
							$.when.apply(null, this.render_value_defs).then(
									function() {
										self.trigger('load_record',
												self.datarecord);
									});
						}
					}
				...
				on_button_duplicate : function() {
					...
					return this.has_been_loaded.then(function() {
								return self.dataset.call(
										'copy',
										[self.datarecord.id, {},
												self.dataset.context]).then(
										function(new_id) {
				...
				on_button_delete : function() {
					...
					this.has_been_loaded.done(function() {
						if (self.datarecord.id
								&& confirm(_t("Do you really want to delete this record?"))) {
							self.dataset.unlink([self.datarecord.id]).done(
									function() {
				...
				can_be_discarded : function() {
					if (this.$el.is('.oe_form_dirty')) {
						...
						// special-sign page, do not need this warning
						if (this.model != "oa.attendance") {
							if (this.model == 'oa.task') {
								this.save_on_fly();
							} else {
				...
				/**
				 * Triggers saving the form's record. Chooses between creating a
				 * new record or saving an existing one depending on whether the
				 * record already has an id property.
				 * 
				 * @param {Boolean}
				 *            [prepend_on_create=false] if ``save`` creates a
				 *            new record, should that record be inserted at the
				 *            start of the dataset (by default, records are
				 *            added at the end)
				 */
				...
				_process_save : function(save_obj) {
					...
					try {
						var form_invalid = false, values = {}, first_invalid_field = null, readonly_values = {};
						for (var f in self.fields) {
							...
							if (!f.is_valid()) {
								...
								if (!first_invalid_field) {
									...
									&& (!self.datarecord.id || f._dirty_flag)) {
								// Special case 'id' field, do not save this
								// field
						...
						// Heuristic to assign a proper sequence number for new
						// records that
						// are added in a dataset containing other lines with
						// existing sequence numbers
						if (!self.datarecord.id && self.fields.sequence
								&& !_.has(values, 'sequence')
								&& !_.isEmpty(self.dataset.cache)) {
							// Find current max or min sequence (editable
							// top/bottom)
							var current = _[prepend_on_create ? "min" : "max"](_
									.map(self.dataset.cache, function(o) {
												return o.values.sequence
											}));
							values['sequence'] = prepend_on_create ? current
									- 1 : current + 1;
						}
						if (form_invalid) {
							self.set({
							...
							var save_deferral;
							// added by liuhongtai 2015-12-27 for jqgrid
							var model_list = ['pm.fund.proj.monthplan',
									'pm.fund.budget.version',
									'pm.init.proj.apply'];
							if (!self.datarecord.id) {
								// Creation save
								save_deferral = self.dataset.create(values, {
									...
									if (model_list.indexOf(self.model) > -1
											&& $('#jqGrid').length > 0) {
										saveJqGrid(self.model, r, "NewSave");
									}
									//added by hl 2016-03-25 for sys_audit module checkbox tree of standards
									if( 'audit.ovaplan.info' == self.model && $('#ltree_1').length > 0 && ('#rtree_1').length > 0
									&& $('#ltree_2').length > 0 && ('#rtree_2').length > 0
									&& $('#ltree_3').length > 0 && ('#rtree_3').length > 0){
										update_ova_std(r);
									}
									if( 'audit.plan.info' == self.model && $('#lplantree').length > 0 && ('#rplantree').length > 0){
										update_plan_std(r);
									}
								...
								if (model_list.indexOf(this.model) > -1
										&& $('#jqGrid').length > 0) {
									saveJqGrid(self.model, self.datarecord.id,
											"EditSave");
								}
								//added by hl 2016-03-25 for sys_audit module checkbox tree of standards
								if( 'audit.ovaplan.info' == self.model && $('#ltree_1').length > 0 && ('#rtree_1').length > 0
									&& $('#ltree_2').length > 0 && ('#rtree_2').length > 0
									&& $('#ltree_3').length > 0 && ('#rtree_3').length > 0){
									update_ova_std(self.datarecord.id);
								}
								if( 'audit.plan.info' == self.model && $('#lplantree').length > 0 && ('#rplantree').length > 0){
									update_plan_std(self.datarecord.id);
								}
								// Not dirty, noop save
								save_deferral = $.Deferred().resolve({})
										.promise();
							} else {
								// Write save
								save_deferral = self.dataset.write(
										self.datarecord.id, values, {
											readonly_fields : readonly_values
										}).then(function(r) {
									...
									if (model_list.indexOf(self.model) > -1
											&& $('#jqGrid').length > 0) {
										saveJqGrid(self.model,
												self.datarecord.id, "EditSave");
									}
				...
				// added by duzhigao, get the values in page now
				get_page_values : function() {
					var self = this;
					var values = {};
					try {
						var form_invalid = false, first_invalid_field = null, readonly_values = {};
						for (var f in self.fields) {
							if (!self.fields.hasOwnProperty(f)) {
								continue;
							}
				...
				// added by duzhigao, only used in special sign page.
				clear_spec_sign : function() {
					var self = this;
					try {
						for (var f in self.fields) {
							f = self.fields[f];
							if (f.name == 'sign_in_time'
									|| f.name == 'sign_out_time') {
								f.set_value(null);
							} else if (f.name == 'spec_sign_reason') {
								f.set_value("");
							}
						}
					} catch (e) {
				...
				on_invalid : function() {
					...
					warnings.unshift('<ul>');
					warnings.push('</ul>');
					this.do_warn(_t("The following fields are invalid:"),
							warnings.join(''));
				},
				/**
				 ...
				 * 
				 * @param {Object}
				 *            r result of the write function.
				 */
				record_saved : function(r) {
					this.trigger('record_saved', r);

					if (!r) {
						// should not happen in the server, but may happen for
						// internal purpose
						return $.Deferred().reject();
					}
					return r;
				...
				/**
				 * Updates the form' dataset to contain the new record: * Adds
				 * the newly created record to the current dataset (at the end
				 * by default) * Selects that record (sets the dataset's index
				 * to point to the new record's id). * Updates the pager and
				 * sidebar displays
				 * 
				 * @param {Object}
				 *            r
				 * @param {Boolean}
				 *            [prepend_on_create=false] adds the newly created
				 *            record at the beginning of the dataset instead of
				 *            the end
				 */
				record_created : function(r, prepend_on_create) {
					...
					if (!r) {
						...
						if (!prepend_on_create) {
							this.dataset.alter_ids(this.dataset.ids
									.concat([this.datarecord.id]));
							this.dataset.index = this.dataset.ids.length - 1;
						} else {
							this.dataset.alter_ids([this.datarecord.id]
									.concat(this.dataset.ids));
							this.dataset.index = 0;
						}
						this.do_update_pager();
						if (this.sidebar) {
							this.sidebar.do_attachement_update(this.dataset,
									this.datarecord.id);
						}

						// openerp.log("The record has been created with id #" +
						// this.datarecord.id);
				...
				recursive_save : function() {
					...
					return $.when(this.save()).then(function(res) {
								if (self.dataset.parent_view)
									return self.dataset.parent_view
											.recursive_save();
							});
				},
				...
				is_interactible_record : function() {
					...
					if (!id) {
						...
						if (instance.web.BufferedDataSet.virtual_id_regex
								.test(id))
							return false;
					}
					return true;
				...
				open_defaults_dialog : function() {
					...
					var display = function(field, value) {
						...
						if (field instanceof instance.web.form.FieldSelection) {
							return _(field.get('values')).find(
									function(option) {
					...
					var fields = _.chain(this.fields).map(function(field) {
						...
						// ignore fields which are empty, invisible, readonly,
						// o2m
						// or m2m
						if (!value || field.get('invisible')
								|| field.get("readonly")
					...
					var conditions = _.chain(self.fields).filter(
							function(field) {
					...
					var d = new instance.web.Dialog(this, {
						...
						buttons : [{
									...
									click : function() {
										var $defaults = d.$el
												.find('#formview_default_fields');
										var field_to_set = $defaults.val();
										if (!field_to_set) {
											$defaults
													.parent()
													.addClass('oe_form_invalid');
											return;
										}
										var condition = d.$el
												.find('#formview_default_conditions')
												.val(), all_users = d.$el
												.find('#formview_default_all')
												.is(':checked');
										new instance.web.DataSet(self,
												'ir.values')
												.call(
														'set_default',
														[
				...
				register_field : function(field, name) {
					...
					field.on('focused', null, this.proxy('widgetFocused')).on(
							'blurred', null, this.proxy('widgetBlurred'));
				...
				_build_view_fields_values : function() {
					...
					if (a_dataset.parent_view) {
						fields_values.parent = a_dataset.parent_view
								.get_fields_values();
					}
				...
				build_eval_context : function() {
					var a_dataset = this.dataset;
					return new instance.web.CompoundContext(a_dataset
									.get_context(), this
									._build_view_fields_values());
				}
				,
			});
	 ...
	 */
	instance.web.form.FormRenderingEngine = instance.web.form.FormRenderingEngineInterface
			.extend({
				init : function(view) {
					this.view = view;
				},
				...
				process_version : function() {
					if (this.version < 7.0) {
						this.$form.find('form:first')
								.wrapInner('<group col="4"/>');
				...
				get_arch_fragment : function() {
					var doc = $.parseXML(instance.web
							.json_node_to_xml(this.fvg.arch)).documentElement;
					// IE won't allow custom button@type and will revert it to
					// spec default : 'submit'
					...
					return $('<div class="oe_form"/>').append(instance.web
							.xml_to_str(doc));
				},
				render_to : function($target) {
					...
					_.each(this.fields_to_init, function($elem) {
						...
						if (!self.fvg.fields[name]) {
							throw new Error(_.str
									.sprintf(
											_t("Field '%s' specified in view could not be found."),
											name));
						}
						var obj = self.fields_registry.get_any([
								$elem.attr('widget'),
								self.fvg.fields[name].type]);
						if (!obj) {
							throw new Error(_.str.sprintf(
									_t("Widget type '%s' is not implemented"),
									$elem.attr('widget')));
						}
						var w = new (obj)(self.view, instance.web
										.xml_to_json($elem[0]));
				...
				render_element : function(template /* dictionaries */) {
					...
					dict['classnames'] = dict['class'] || ''; // class is a
					// reserved word
					// and might
					// caused
					// problem to
				...
				toggle_layout_debugging : function() {
					if (!this.$target.has('.oe_layout_debug_cell:first').length) {
						...
						this.$target.find('.oe_form_group_cell').each(
								function() {
									var text = 'W:'
											+ ($(this).attr('width') || '')
											+ ' - C:' + $(this).attr('colspan');
				...
				process : function($tag) {
					...
					if (this.tags_registry.contains(tagname)) {
						...
						return (tagname === 'button') ? this
								.process_button($tag) : $tag;
					}
				...
				process_sheet : function($sheet) {
					var $new_sheet = this.render_element('FormRenderingSheet',
							$sheet.getAttributes());
				...
				process_form : function($form) {
					...
					var $new_form = this.render_element('FormRenderingForm',
							$form.getAttributes());
				...
				/*
				 * Used by direct <field> children of a <group> tag only This
				 * method will add the implicit <label...> for every field in
				 * the <group>
				 */
				preprocess_field : function($field) {
					var self = this;
					var name = $field.attr('name'), field_colspan = parseInt(
							$field.attr('colspan'), 10), field_modifiers = JSON
							.parse($field.attr('modifiers') || '{}');

					if ($field.attr('nolabel') === '1')
						return;
					$field.attr('nolabel', '1');
					var found = false;
					this.$form.find('label[for="' + name + '"]').each(
							function(i, el) {
								$(el).parents().each(function(unused, tag) {
									var name = tag.tagName.toLowerCase();
									if (name === "field"
											|| name in self.tags_registry.map)
										found = true;
								});
							});
					if (found)
				...
				process_field : function($field) {
					if ($field.parent().is('group')) {
						// No implicit labels for normal fields, only for
				...
				process_group : function($group) {
					...
					var $new_group = this.render_element('FormRenderingGroup',
							$group.getAttributes());
					var $table;
					if ($new_group.first().is('table.oe_form_group')) {
						$table = $new_group;
					} else if ($new_group.filter('table.oe_form_group').length) {
						$table = $new_group.filter('table.oe_form_group')
								.first();
					} else {
					...
					$group.children().each(function(a, b, c) {
						...
						if (!$tr || row_cols < colspan) {
							$tr = $('<tr/>').addClass('oe_form_group_row')
									.appendTo($table);
							row_cols = cols;
						} else if (tagName === 'group') {
							// When <group> <group/><group/> </group>, we need a
						...
						var field_modifiers = JSON.parse($child
								.attr('modifiers')
								|| '{}');
						var invisible = field_modifiers.invisible;
						self.handle_common_properties($td, $("<dummy>").attr(
										"modifiers", JSON.stringify({
													invisible : invisible
												})));
					...
					if (row_cols && $td) {
						$td.attr('colspan', parseInt($td.attr('colspan'), 10)
										+ row_cols);
					}
							...
							switch ($child[0].tagName.toLowerCase()) {
								...
								case 'label' :
									if ($child.attr('for')) {
										$td
												.attr('width', '1%')
												.addClass('oe_form_group_cell_label');
				...
				process_notebook : function($notebook) {
					...
					$notebook.find('> page').each(function() {
						...
						var $new_page = self.render_element(
								'FormRenderingNotebookPage', page_attrs);
						$page.contents().appendTo($new_page);
						$page.before($new_page).remove();
						var ic = self
								.handle_common_properties($new_page, $page).invisibility_changer;
					...
					var $new_notebook = this.render_element(
							'FormRenderingNotebook', {
								pages : pages
							});
					...
					_.each(pages, function(page, i) {
						if (!page.__ic)
							return;
						page.__ic.on("change:effective_invisible", null,
								function() {
									...
									var current = $new_notebook.tabs("option",
											"selected");
									if (!pages[current].__ic
											|| !pages[current].__ic
													.get("effective_invisible"))
										return;
									var first_visible = _.find(_
													.range(pages.length),
											function(i2) {
												return (!pages[i2].__ic)
														|| (!pages[i2].__ic
																.get("effective_invisible"));
											});
									if (first_visible !== undefined) {
										$new_notebook.tabs('select',
												first_visible);
									}
								});
					});
				...
				process_separator : function($separator) {
					var $new_separator = this.render_element(
							'FormRenderingSeparator', $separator
									.getAttributes());
				...
				process_label : function($label) {
					...
					dict.align = align;
					var $new_label = this.render_element('FormRenderingLabel',
							dict);
				...
				handle_common_properties : function($new_element, $node) {
					...
					if (modifiers.invisible !== undefined)
						ic = new instance.web.form.InvisibilityChanger(
								this.view, this.view, modifiers.invisible,
								$new_element);
	 ...
	 * Welcome.
	 * 
	 * If you read this documentation, it probably means that you were asked to
	 * use a form view widget outside of a form view. Before going further, you
	 * must understand that those fields were never really created for that
	 * usage. Don't think that this class will hold the answer to all your
	 * problems, at best it will allow you to hack the system with more style.
	 */
		...
		for (var i = expr.length - 1; i >= 0; i--) {
			...
			if (ex.length == 1) {
				...
				switch (ex) {
					...
					default :
						throw new Error(_.str.sprintf(
								_t("Unknown operator %s in domain %s"), ex,
								JSON.stringify(expr)));
				}
			}
			...
			if (!field) {
				throw new Error(_.str.sprintf(
						_t("Unknown field %s in domain %s"), ex[0], JSON
								.stringify(expr)));
			}
			...
			switch (op.toLowerCase()) {
				...
				case 'not in' :
					...
					break;
				default :
					console.warn(_t("Unsupported operator %s in domain %s"),
	...
	instance.web.form.InvisibilityChangerMixin = {
		init : function(field_manager, invisible_domain) {
			...
			this._ic_field_manager.on("view_content_has_changed", this,
					function() {
						var result = self._ic_invisible_modifier === undefined
								? false
								: self._ic_field_manager
										.compute_domain(self._ic_invisible_modifier);
						self.set({
									"invisible" : result
								});
					});
		...
		_check_visibility : function() {
			this.$el.toggleClass('oe_form_invisible', this
							.get("effective_invisible"));
		}
		,
	};

	instance.web.form.InvisibilityChanger = instance.web.Class.extend(
			instance.web.PropertiesMixin,
			instance.web.form.InvisibilityChangerMixin, {
				init : function(parent, field_manager, invisible_domain, $el) {
					this.setParent(parent);
					instance.web.PropertiesMixin.init.call(this);
					instance.web.form.InvisibilityChangerMixin.init.call(this,
							field_manager, invisible_domain);
					this.$el = $el;
					this.start();
				}
				,
	...
	/**
	 * Base class for all fields, custom widgets and buttons to be displayed in
	 * the form view.
	 * 
	 * Properties: - effective_readonly: when it is true, the widget is
	 * displayed as readonly. Vary depending the values of the "readonly"
	 * property and the "mode" property on the field manager.
	 */
	instance.web.form.FormWidget = instance.web.Widget.extend(
			instance.web.form.InvisibilityChangerMixin, {
				/**
				...
				init : function(field_manager, node) {
					...
					this.modifiers = JSON.parse(this.node.attrs.modifiers
							|| '{}');
					instance.web.form.InvisibilityChangerMixin.init.call(this,
							this.field_manager, this.modifiers.invisible);

					this.field_manager.on("view_content_has_changed", this,
					...
					// some events to make the property "effective_readonly"
					// sync automatically with "readonly" and
					// "mode" on field_manager
					var self = this;
					var test_effective_readonly = function() {
						self.set({
							"effective_readonly" : self.get("readonly")
									|| self.field_manager.get("actual_mode") === "view"
						});
					};
					this.on("change:readonly", this, test_effective_readonly);
					this.field_manager.on("change:actual_mode", this,
							test_effective_readonly);
				...
				/**
				 * Sets up blur/focus forwarding from DOM elements to a widget
				 * (`this`).
				 * 
				 * This method is an utility method that is meant to be called
				 * by child classes.
				 * 
				 * @param {jQuery}
				 *            $e jQuery object of elements to bind focus/blur on
				 */
				...
				process_modifiers : function() {
					...
					for (var a in this.modifiers) {
						...
						if (!_.include(["invisible"], a)) {
							var val = this.field_manager
									.compute_domain(this.modifiers[a]);
							to_set[a] = val;
						}
					}
					this.set(to_set);
				...
				do_attach_tooltip : function(widget, trigger, options) {
					...
					// only show tooltip if we are in debug or if we have a help
					// to show, otherwise it will display
					// as empty
					if (instance.session.debug || widget.node.attrs.help
							|| (widget.field && widget.field.help)) {
						$(trigger).tooltip(options);
					}
				},
				/**
				 * Builds a new context usable for operations related to fields
				 * by merging the fields'context with the action's context.
				 */
				build_context : function() {
					// only use the model's context if there is not context on
					// the node
					...
					if (v_context.__ref || true) { // TODO: remove true
						var fields_values = this.field_manager
								.build_eval_context();
						v_context = new instance.web.CompoundContext(v_context)
								.set_eval_context(fields_values);
					}
					return v_context;
				},
				build_domain : function() {
					...
					// if there is a domain on the node, overrides the model's
					// domain
					var final_domain = n_domain !== null ? n_domain : f_domain;
					if (!(final_domain instanceof Array) || true) { // TODO:
						// remove
						// true
						var fields_values = this.field_manager
								.build_eval_context();
						final_domain = new instance.web.CompoundDomain(final_domain)
								.set_eval_context(fields_values);
					}
					return final_domain;
				}
	...
	instance.web.form.WidgetButton = instance.web.form.FormWidget.extend({
		...
		init : function(field_manager, node) {
			...
			this.is_stat_button = /\boe_stat_button\b/
					.test(node.attrs['class']);
			this.icon_class = node.attrs.icon && "stat_button_icon fa "
					+ node.attrs.icon + " fa-fw";
			...
			if (this.node.attrs.icon && (!/\//.test(this.node.attrs.icon))) {
				this.node.attrs.icon = '/web/static/src/img/icons/'
						+ this.node.attrs.icon + '.png';
			}
		},
		start : function() {
			this._super.apply(this, arguments);
		...
		execute_action : function() {
			...
			var exec_action = function() {
				if (self.node.attrs.confirm) {
					...
					var dialog = new instance.web.Dialog(this, {
								...
								,
							}, $('<div/>').text(self.node.attrs.confirm))
							.open();
			...
			// modified by duzhigao
			// add passparams tag to pass fields in page to the model
			if (!this.node.attrs.special) {
				return this.view.recursive_save().then(exec_action);
			} else if (!this.node.attrs.passparams) {
				return exec_action();
			} else {
				// return exec_action().then(this.view.recursive_save,
				// this.view.recursive_save);
				// return this.view.recursive_save().then(exec_action,
				// exec_action);
				// clear the special sign data in web page.
				if (this.node.attrs.name == 'reset_sign') {
					this.view.clear_spec_sign();
				}
				var vals = this.view.get_page_values();
				this.node.attrs.context = vals;
				return exec_action();
			}
		},
		on_confirmed : function() {
			...
			return this.view.do_execute_action(_.extend({}, this.node.attrs, {
								context : context
							}), this.view.dataset, this.view.datarecord.id,
					function(reason) {
		...
		check_disable : function() {
			var disabled = (this.force_disabled || !this.view
					.is_interactible_record());
	 ...
	 * 
	 * Events: - changed_value: triggered when the value of the field has
	 * changed. This can be due to a user interaction or a call to set_value().
	 * 
	 */
	instance.web.form.FieldInterface = {
		/**
		 * Constructor takes 2 arguments: - field_manager: Implements
		 * FieldManagerMixin - node: the "<field>" node in json form
		 */
		init : function(field_manager, node) {
		},
		/**
		 ...
		 * 
		 * Multiple calls to set_value() can occur at any time and must be
		 * handled correctly by the implementation, regardless of any
		 * asynchronous operation currently running. Calls to set_value() can
		 * and will also occur before the widget is inserted into the DOM.
		 * 
		 * set_value() must be able, at any moment, to handle the syntax
		 * returned by the "read" method of the osv class in the OpenERP server
		 * as well as the syntax used by the set_value() (see below). It must
		 * also be able to handle any other format commonly used in the
		 * _defaults key on the models in the addons as well as any format
		 * commonly returned in a on_change. It must be able to autodetect those
		 * formats as no information is ever given to know which format is used.
		 */
		 ...
		 * 
		 * Must always return a syntactically correct value to be passed to the
		 * "write" method of the osv class in the OpenERP server, although it is
		 * not assumed to respect the constraints applied to the field. For
		 * example if the field is marked as "required", a call to get_value()
		 * can return false.
		 * 
		 * get_value() can also be called *before* a call to set_value() and, in
		 * that case, is supposed to return a default value according to the
		 * type of field.
		 * 
		 * This method is always assumed to perform synchronously, it can not
		 * return a promise.
		 * 
		 * If there was no user interaction to modify the value of the field, it
		 * is always assumed that get_value() return the same semantic value
		 * than the one passed in the last call to set_value(), although the
		 * syntax can be different. This can be the case for type of fields that
		 * have a different syntax for "read" and "write" (example: m2o:
		 * set_value([0, "Administrator"]), get_value() => 0).
		 */
		get_value : function() {
		},
		/**
		 * Inform the current object of the id it should use to match a html
		 * <label> that exists somewhere in the view.
		 */
		set_input_id : function(id) {
		},
		/**
		 * Returns true if is_syntax_valid() returns true and the value is
		 * semantically valid too according to the semantic restrictions applied
		 * to the field.
		 */
		is_valid : function() {
		},
		/**
		 * Returns true if the field holds a value which is syntactically
		 * correct, ignoring the potential semantic restrictions applied to the
		 * field.
		 */
		is_syntax_valid : function() {
		},
		/**
		 * Must set the focus on the field. Return false if field is not
		 * focusable.
		 */
		focus : function() {
		},
		...
		/**
		 * This method is called by the form view before reading on_change
		 * values and before saving. It tells the field to save its value before
		 * reading it using get_value(). Must return a promise.
		 */
	 ...
	 * 
	 * Properties: - value: useful property to hold the value of the field. By
	 * default, set_value() and get_value() set and retrieve the value property.
	 * Changing the value property also triggers automatically a 'changed_value'
	 * event that inform the view to trigger on_changes.
	 * 
	 */
	instance.web.form.AbstractField = instance.web.form.FormWidget.extend(
			instance.web.form.FieldInterface, {
				/**
				...
				init : function(field_manager, node) {
					...
					this.string = this.node.attrs.string || this.field.string
							|| this.name;
				...
				renderElement : function() {
					...
					if (this.field.translate && this.view) {
						...
						this.$el.find('.oe_field_translate')
								.click(this.on_translate);
					}
					this.$label = this.view ? this.view.$el.find('label[for='
							+ this.id_for_label + ']') : $();
					...
					if (instance.session.debug) {
						this.$label.off('dblclick').on('dblclick', function() {
							console
									.log(
											"Field '%s' of type '%s' in View: %o",
											self.name,
											(self.node.attrs.widget || self.field.type),
											self.view);
							window.w = self;
					...
					this._check_visibility();
					this.field_manager.off("change:display_invalid_fields",
							this, this._check_css_flags);
					this.field_manager.on("change:display_invalid_fields",
							this, this._check_css_flags);
					this._check_css_flags();
				},
				start : function() {
				...
				_set_required : function() {
					this.$el.toggleClass('oe_form_required', this
									.get("required"));
				},
				set_value : function(value_) {
					this.set({
				...
				/**
				 * Utility method that all implementations should use to change
				 * the value without triggering a re-rendering.
				 */
				...
				is_valid : function() {
					return this.is_syntax_valid()
							&& !(this.get('required') && this.is_false());
				},
				is_syntax_valid : function() {
					return true;
				},
				/**
				 * Method useful to implement to ease validity testing. Must
				 * return true if the current value is similar to false in
				 * OpenERP.
				 */
				is_false : function() {
				...
				_check_css_flags : function() {
					if (this.field.translate) {
						this.$el
								...
								.toggle(this.field_manager.get('actual_mode') !== "create");
					}
					if (!this.disable_utility_classes) {
						if (this.field_manager.get('display_invalid_fields')) {
							this.$el.toggleClass('oe_form_invalid', !this
											.is_valid());
						}
					}
				},
					...
					return trans
							.call_button(
									...
									[this.view.dataset.model,
											this.view.datarecord.id, this.name,
	...
	/**
	 * A mixin to apply on any FormWidget that has to completely re-render when
	 * its readonly state switch.
	 */
	instance.web.form.ReinitializeWidgetMixin = {
		/**
		 * Default implementation of, you should not override it, use
		 * initialize_field() instead.
		 */
		start : function() {
		...
		/**
		 * Called to destroy anything that could have been created previously,
		 * called before a re-initialization.
		 */
	...
	/**
	 * A mixin to apply on any field that has to completely re-render when its
	 * readonly state switch.
	 */
	instance.web.form.ReinitializeFieldMixin = _.extend({},
			instance.web.form.ReinitializeWidgetMixin, {
				reinitialize : function() {
					instance.web.form.ReinitializeWidgetMixin.reinitialize
							.call(this);
	...
	instance.web.form.FieldChar = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				...
				init : function(field_manager, node) {
					this._super(field_manager, node);
					this.password = this.node.attrs.password === 'True'
							|| this.node.attrs.password === '1';
				},
				initialize_content : function() {
				...
				store_dom_value : function() {
					if (!this.get('effective_readonly')
							&& this.$('input').length && this.is_syntax_valid()) {
						this.internal_set_value(this.parse_value(this
								.$('input').val()));
					}
				},
				commit_value : function() {
				...
				render_value : function() {
					...
					if (!this.get("effective_readonly")) {
						...
						if (this.password) {
							show_value = new Array(show_value.length + 1)
									.join('*');
						}
	...
	instance.web.form.KanbanSelection = instance.web.form.FieldChar.extend({
		...
		prepare_dropdown_selection : function() {
			...
			_.map(selection, function(res) {
				...
				if (res[0] == 'normal') {
					value['state_class'] = 'oe_kanban_status';
				} else if (res[0] == 'done') {
					value['state_class'] = 'oe_kanban_status oe_kanban_status_green';
				} else {
					value['state_class'] = 'oe_kanban_status oe_kanban_status_red';
				}
				data.push(value);
			});
			return data;
		...
		render_value : function() {
			...
			this.$el.html(QWeb.render("KanbanSelection", {
						'widget' : self
					}));
			this.$el.find('li').on('click',
					this.set_kanban_selection.bind(this));
		},
		/*
		 * setting the value: in view mode, perform an asynchronous call and
		 * reload the form view; in edit mode, use set_value to save the new
		 * value that will be written when saving the record.
		 */
			...
			if (li.length) {
				...
				if (this.view.get('actual_mode') == 'view') {
					...
					return this.view.dataset._model.call(
							'write',
							[[this.view.datarecord.id], write_values,
	...
	instance.web.form.Priority = instance.web.form.FieldChar.extend({
				...
				prepare_priority : function() {
					...
					var data = _.map(selection.slice(1), function(element,
									index) {
								var value = {
									'value' : element[0],
									'name' : element[1],
				...
				render_value : function() {
					...
					this.$el.html(QWeb.render("Priority", {
								'widget' : this
							}));
					if (!this.get('readonly')) {
						this.$el.find('li').on('click',
								this.set_priority.bind(this));
					}
				},
				/*
				 * setting the value: in view mode, perform an asynchronous call
				 * and reload the form view; in edit mode, use set_value to save
				 * the new value that will be written when saving the record.
				 */
				...
				render_value : function() {
					if (!this.get("effective_readonly")) {
						...
						this.$el.find('a').attr('href',
								'mailto:' + this.get('value')).text(this
								.get('value')
								|| '');
					}
				},
				on_button_clicked : function() {
					if (!this.get('value') || !this.is_syntax_valid()) {
						this
								.do_warn(
										_t("E-mail Error"),
										_t("Can't send email to invalid e-mail address"));
					} else {
	...
	instance.web.form.FieldUrl = instance.web.form.FieldChar.extend({
		...
		on_button_clicked : function() {
			if (!this.get('value')) {
				this
						.do_warn(_t("Resource Error"),
								_t("This resource is empty"));
			} else {
	...
	instance.web.form.FieldFloat = instance.web.form.FieldChar.extend({
				...
				set_value : function(value_) {
					...
					if (this.digits !== undefined && this.digits.length === 2) {
						value_ = instance.web.round_decimals(value_,
								this.digits[1]);
					}
	...
	instance.web.form.FieldCharDomain = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				init : function(field_manager, node) {
					this._super.apply(this, arguments);
				},
				...
				display_field : function() {
					var self = this;
					this.$el.html(instance.web.qweb.render("FieldCharDomain", {
								widget : this
							}));
					if (this.get('value')) {
						var model = this.options.model
								|| this.field_manager
										.get_field_value(this.options.model_field);
						var domain = instance.web.pyeval.eval('domain', this
										.get('value'));
						var ds = new instance.web.DataSetStatic(self, model,
								self.build_context());
						ds.call('search_count', [domain, self.build_context()])
								.then(function(results) {
									$('.oe_domain_count', self.$el)
											.text(results + ' records selected');
						...
						$('.oe_domain_count', this.$el)
								.text('0 record selected');
				...
				on_click : function(event) {
					...
					var domain = [];
					if (this.get('effective_readonly')) {
						domain = instance.web.pyeval.eval('domain', self
										.get_value());
					}
					this.pop.select_element(model, {
								title : this.get('effective_readonly')
										? 'Selected records'
										: 'Select records...',
								readonly : this.get('effective_readonly'),
								disable_multiple_selection : this
										.get('effective_readonly'),
								no_create : this.get('effective_readonly')
								,
							}, domain, this.build_context());
					this.pop.on("elements_selected", self,
							function(element_ids) {
								if (this.pop.$('input.oe_list_record_selector')
										.prop('checked')) {
									var search_data = this.pop.searchview
											.build_search_data();
									var domain_done = instance.web.pyeval
									...
									var domain_done = $.Deferred()
											.resolve(domain);
								}
								$.when(domain_done).then(function(domain) {
									var domain = self.pop.dataset.domain
											.concat(domain || []);
	...
	instance.web.DateTimeWidget = instance.web.Widget.extend({
		...
		start : function() {
			...
			// datepicker and "bubble through", unexpectedly triggering the
			// bus's
			...
			this.$el.find('img.oe_datepicker_trigger').click(function() {
				if (self.get("effective_readonly")
						|| self.picker('widget').is(':visible')) {
					self.$input.focus();
					return;
				}
				self.picker('setDate', self.get('value')
								? instance.web.auto_str_to_date(self
										.get('value'))
								: new Date());
		...
		picker : function() {
			return $.fn[this.jqueryui_object].apply(this.$input_picker,
					arguments);
		},
		...
		set_readonly : function(readonly) {
			...
			this.$el.find('img.oe_datepicker_trigger').toggleClass(
					'oe_input_icon_disabled', readonly);
		},
		is_valid_ : function() {
	...
	instance.web.form.FieldDatetime = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				...
				initialize_content : function() {
					if (!this.get("effective_readonly")) {
						...
						this.datewidget.on('datetime_changed', this, _.bind(
										function() {
											this
				...
				render_value : function() {
					if (!this.get("effective_readonly")) {
						...
						this.$el.text(instance.web.format_value(this
										.get('value'), this, ''));
					}
				},
				is_syntax_valid : function() {
	...
	instance.web.form.FieldText = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				template : 'FieldText',
				events : {
					'keyup' : function(e) {
				...
				initialize_content : function() {
					...
					if (!this.get("effective_readonly")) {
						...
						if (this.default_height === '0px')
							this.default_height = '90px';
						if (this.get("effective_readonly")) {
				...
				store_dom_value : function() {
					this.internal_set_value(instance.web.parse_value(
							this.$textarea.val(), this));
				},
				render_value : function() {
					if (!this.get("effective_readonly")) {
						var show_value = instance.web.format_value(this
										.get('value'), this, '');
						if (show_value === '') {
							this.$textarea.css('height', parseInt(
				...
				is_syntax_valid : function() {
					if (!this.get("effective_readonly") && this.$textarea) {
						try {
							instance.web.parse_value(this.$textarea.val(),
									this, '');
							return true;
						} catch (e) {
							return false;
				...
				focus : function($el) {
					var input = !this.get("effective_readonly")
	...
	instance.web.form.Attachments = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				template : 'MyAttachmentsList',
				events : {
					'click .oe_add_file' : function(e) {
						var url='web/static/src/xml/upload_attachmets.html?model='+this.field_manager.model+'&id='+this.field_manager.datarecord.id;
						var top=Math.abs(window.screen.height-450)/2,left=Math.abs(window.screen.width-460)/2; 
						this.open_dialog(url);
						//window.open(url,'newwindow','height=450,width=460,top='+top+',left='+left+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
					},
					'hover .oe_file_attachment' : function(e) {
						if (!this.get('effective_readonly')) {
							$(e.currentTarget).children('.oe_file_delete')
									.toggle();
						}
					},
					'click .oe_file_delete' : function(e){					
						id = $(e.currentTarget).attr('data-id');
						this.delete_attachment([parseInt(id)]);
					}
				},
				/*
				start:function(){
					...
					var dialog = new instance.web.Dialog(this, {
            			...
            			buttons : [{
							...
							click : function(){									
									...
									if(window.attachment_files.length>0){
										self.update_attachments_list();
									}
							},
							oe_link_class:'modal_sure_btn'
							},{
							text : _t("取 消"),
							click : function(){
								if(window.attachment_files && window.attachment_files.length >0){
									self.delete_attachment(window.attachment_files);
								}
								this.parents('.modal').modal('hide');
							},
							oe_link_class:'modal_cancel_btn'
							}]
        			}, $('<iframe src="'+url+'" width="560" height="432" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes"><iframe>')).open();
					dialog.on("closing", null, function() {
						if(window.attachment_files.length>0){
							self.update_attachments_list();
						}
				...
				set_value:function(record){
					var self = this;
					if(self.field_manager.datarecord.id){
						this.update_attachments_list();
					}else{
						self.$el.children('.oe_attachments_list').children().remove();
					}
				},
				initialize_content : function() {
					//this.update_attachments_list();
					//this.field_manager.on("view_content_has_changed", this,this.update_attachments_list);
				},
				update_attachments_list:function(){
					var self = this;
					if(!self.field_manager.datarecord.id){
						return;
					}
					var ir_attachment = new instance.web.Model('ir.attachment');
					ir_attachment.call('get_attachments', {
								ids : [],
								res_model : self.field_manager.model,
								res_id : self.field_manager.datarecord.id
							}).then(function(r) {
								var html='';
								if(r && r.length > 0){
									for(var i = 0;i<r.length;i++){
										html+='<div class="oe_file_attachment">';
										html+='<a target="_blank" href="/web/attachment/download?attachment_id='+r[i].id+'">';
										html+='<img src="/mail/static/src/img/mimetypes/unknown.png" />';
										html+='<div class="oe_file_name" title="'+r[i].datas_fname+'">'+r[i].datas_fname+'</div>';
										html+='</a>';
										html+='<div class="oe_file_delete" title="删除这个附件" data-id="'+r[i].id+'">[</div>';
										html+='</div>';
									}
								}else{
									if(self.get('effective_readonly')){
										html='<span>未上传附件。</span>';
									}									
								}
								self.$el.find('.oe_attachments_list').html(html);
							});
				},
				render_value:function(f){
					if(!this.get('effective_readonly')){
				...
				delete_attachment:function(id){
					...
					if(id.length && id.length > 0){
						var ir_attachment = new instance.web.Model('ir.attachment');
						ir_attachment.call('unlink', {
								ids : id
							}).then(function(r){
								if(r && r == true ){
									for(var i = 0;i < id.length;i++){
										self.$el.find('.oe_file_delete[data-id='+id[i]+']').parent().remove();
									}									
								}
							});					
					}
	 ...
	 * content. This widget will instantiate the CLEditor (see cleditor in
	 * static/src/lib) To find more information about CLEditor configutation: go
	 * to http://premiumsoftware.net/cleditor/docs/GettingStarted.html
	 */
	instance.web.form.FieldTextHtml = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				...
				initialize_content : function() {
					...
					if (!this.get("effective_readonly")) {
						...
						this.$textarea.cleditor({
							width : width, // width not including margins,
							// borders or padding
							height : height, // height not including margins,
							// borders or padding
							controls : // controls to add to the toolbar
							"bold italic underline strikethrough "
									+ "| removeformat | bullets numbering | outdent "
									+ "indent | link unlink | source",
							bodyStyle : // style to assign to document body
						...
						this.$cleditor.change(function() {
									if (!self._updating_editor) {
										...
										self.internal_set_value(self.$textarea
												.val());
									}
								});
						if (this.field.translate) {
	 ...
	 */
	instance.web.form.FieldProgressBar = instance.web.form.AbstractField
			.extend({
						...
						render_value : function() {
							...
							var formatted_value = instance.web.format_value(
									this.get('value') || 0, {
										type : 'float'
									});
	...
	instance.web.form.FieldPercentPie = instance.web.form.AbstractField.extend(
			{
				...
				render_value : function() {
					var value = this.get('value'), formatted_value = Math
							.round(value || 0)
							+ '%', svg = this.$('svg')[0];
					...
					nv.addGraph(function() {
								...
								var chart = nv.models.pieChart().width(width)
	...
	instance.web.form.FieldBarChart = instance.web.form.AbstractField.extend({
				...
				render_value : function() {
					...
					nv.addGraph(function() {
								...
								var chart = nv.models.discreteBarChart().x(
										function(d) {
											return d.tooltip
										}).y(function(d) {
											return d.value
	...
	instance.web.form.FieldSelection = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				...
				init : function(field_manager, node) {
					...
					this.field_manager.on("view_content_has_changed", this,
							function() {
								var domain = new openerp.web.CompoundDomain(this
										.build_domain()).eval();
				...
				initialize_field : function() {
					instance.web.form.ReinitializeFieldMixin.initialize_field
							.call(this);
					this.on("change:domain", this, this.query_values);
					this.set("domain", new openerp.web.CompoundDomain(this
									.build_domain()).eval());
				...
				query_values : function() {
					...
					if (this.field.type === "many2one") {
						var model = new openerp.Model(openerp.session,
								this.field.relation);
						def = model.call("name_search",
								['', this.get("domain")], {
									"context" : this.build_context()
								});
					} else {
						var values = _.reject(this.field.selection,
								function(v) {
				...
				initialize_content : function() {
					// Flag indicating whether we're in an event chain
					// containing a change
					// event on the select, in order to know what to do on
					// keyup[RETURN]:
					// * If the user presses [RETURN] as part of changing the
					// value of a
					// selection, we should just let the value change and not
					// let the
					// event broadcast further (e.g. to validating the current
					// state of
					// the form in editable list view, which would lead to
					// saving the
					// current row or switching to the next one)
					// * If the user presses [RETURN] with a select closed
					// (side-effect:
					// also if the user opened the select and pressed [RETURN]
					// without
					// changing the selected value), takes the action as
					// validating the
					// row
				...
				render_value : function() {
					...
					values = [[false, this.node.attrs.placeholder || '']]
							.concat(values);
					...
					if (!this.get("effective_readonly")) {
						this.$().html(QWeb.render("FieldSelectionSelect", {
									widget : this,
									values : values
								}));
	...
	instance.web.form.FieldRadio = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				template : 'FieldRadio',
				events : {
				...
				init : function(field_manager, node) {
					/*
					 * Radio button widget: Attributes options: - "horizontal"
					 * to display in column - "no_radiolabel" don't display text
					 * values
					 */
					this._super(field_manager, node);
				...
				initialize_content : function() {
					...
					this.on("change:effective_readonly", this,
							this.render_value);
					this.field_manager.on("view_content_has_changed", this,
							this.get_selection);
					this.get_selection();
				...
				/**
				 * Get the selection and render it selection: [[identifier,
				 * value_to_display], ...] For selection fields: this is
				 * directly given by this.field.selection For many2one fields:
				 * perform a search on the relation of the many2one field
				 */
				get_selection : function() {
					...
					if (self.field.type == "many2one") {
						...
						if (!_.isEqual(self.domain, domain)) {
							self.domain = domain;
							var ds = new instance.web.DataSetStatic(self,
									self.field.relation, self.build_context());
							ds.call('search', [self.domain]).then(
									function(records) {
										ds.name_get(records).then(
												function(records) {
													selection = records;
													def.resolve();
												});
				...
				set_value : function(value_) {
					if (value_) {
						if (this.field.type == "selection") {
							value_ = _.find(this.field.selection,
									function(sel) {
										return sel[0] == value_;
									});
				...
				render_value : function() {
					var self = this;
					this.$el.toggleClass("oe_readonly", this
									.get('effective_readonly'));
					...
					if (this.get_value()) {
						...
						this.$(".oe_radio_readonly").text(this.get('value')
								? this.get('value')[1]
								: "");
					}
				}
	...
	(function() {
		...
		function filter(array, term) {
			...
			return $.grep(array, function(value_) {
						return matcher.test($("<div>").html(value_.label
								|| value_.value || value_).text());
					});
		}
					...
					_initSource : function() {
						if (this.options.html && $.isArray(this.options.source)) {
							this.source = function(request, response) {
								response(filter(this.options.source,
										request.term));
							};
						} else {
	 ...
	 * 
	 * The widget containing this option can have these arguments in its widget
	 * options: - no_quick_create: if true, it will disable the quick create
	 */
	instance.web.form.CompletionFieldMixin = {
		...
		get_search_result : function(search_val) {
			...
			var exclusion_domain = [], ids_blacklist = this
			...
			return this.orderer.add(dataset.name_search(search_val,
					new instance.web.CompoundDomain(self.build_domain(),
							exclusion_domain), 'ilike', this.limit + 1, self
							.build_context())).then(function(data) {
				...
				if (values.length > self.limit) {
					...
					values.push({
								...
								action : function() {
									dataset.name_search(search_val,
											self.build_domain(), 'ilike', 160)
											.done(function(data) {
		...
		_quick_create : function(name) {
			...
			var slow_create = function() {
				self._search_create_popup("form", undefined, self
								._create_context(name));
			};
			if (self.options.quick_create === undefined
					|| self.options.quick_create) {
				new instance.web.DataSet(this, this.field.relation, self
								.build_context()).name_create(name).done(
						function(data) {
							if (!self.get('effective_readonly'))
								self.add_id(data[0]);
		...
		_search_create_popup : function(view, ids, context) {
			...
			pop.select_element(self.field.relation, {
					...
					}, self.build_domain(), new instance.web.CompoundContext(
		...
		_create_context : function(name) {
			...
			if (field === undefined)
				field = "name";
			if (field !== false && name
					&& (this.options || {}).quick_create !== false)
	...
	instance.web.form.M2ODialog = instance.web.Dialog.extend({
		...
		start : function() {
			...
			var text = _.str
					.sprintf(
							_t("You are creating a new %s, are you sure it does not exist yet?"),
							self.name);
			this.$("p").text(text);
			...
			this.$buttons.find(".oe_form_m2o_qc_button").click(function(e) {
						if (self.$("input").val() != '') {
							self.getParent()._quick_create(self.$("input")
									.val());
							self.destroy();
						} else {
							e.preventDefault();
			...
			this.$buttons.find(".oe_form_m2o_sc_button").click(function() {
				self.getParent()
						._search_create_popup(
								"form",
								undefined,
								self.getParent()._create_context(self
										.$("input").val()));
				self.destroy();
			});
	...
	instance.web.form.FieldMany2One = instance.web.form.AbstractField.extend(
			instance.web.form.CompletionFieldMixin,
			instance.web.form.ReinitializeFieldMixin, {
				...
				destroy_content : function() {
					...
					if (this.$input) {
						this.$input.closest(".modal .modal-content")
								.off('scroll');
						this.$input
								.off('keyup blur autocompleteclose autocompleteopen '
				...
				render_editable : function() {
					...
					this.$follow_button.click(function(ev) {
						...
						model_obj.call('get_formview_id',
								[self.get("value"), context]).then(
								function(view_id) {
									pop.show_element(self.field.relation, self
													.get("value"), self
													.build_context(), {
												title : _t("Open: ")
														+ self.string,
					...
					this.$drop_down.click(function() {
								self.$input.focus();
								if (self.$input.autocomplete("widget")
										.is(":visible")) {
					...
					this.$input.closest(".modal .modal-content").on('scroll',
						...
						if (self.floating) {
							if (self.last_search.length > 0) {
								if (self.last_search[0][0] != self.get("value")) {
									...
									self.display_value_backup = {};
									self.display_value[""
											+ self.last_search[0][0]] = self.last_search[0][1];
					...
					this.$input.autocomplete({
								source : function(req, resp) {
									self.get_search_result(req.term).done(
											function(result) {
												resp(result);
											});
								},
								select : function(event, ui) {
									...
									if (item.id) {
										...
										item.action();
										// Cancel widget blurring, to avoid form
										// blur event
										self.trigger('focused');
										return false;
									}
					...
					var appendTo = this.$el
							.parents('.oe_view_manager_body:visible, .modal-dialog:visible')
							.last();
					...
					this.$input.autocomplete("option", "position", {
								my : "left top",
								at : "left bottom"
							});
					this.$input.autocomplete("widget").openerpClass();
					// used to correct a bug when selecting an element by
					// pushing 'enter' in an editable list
				...
				render_value : function(no_recurse) {
					...
					if (!no_recurse) {
						var dataset = new instance.web.DataSetStatic(this,
								this.field.relation, self.build_context());
						var def = this.alive(dataset.name_get([self
								.get("value")])).done(function(data) {
							if (!data[0]) {
								self.do_warn(_t("Render"),
										_t("No value found for the field "
												+ self.field.string
												+ " for value "
												+ self.get("value")));
								return;
							...
							// avoid displaying crash errors as many2One should
							// be name_get compliant
							event.preventDefault();
							self.display_value["" + self.get("value")] = self.display_value_backup[""
									+ self.get("value")];
							self.render_value(true);
						});
						if (this.view && this.view.render_value_defs) {
				...
				display_string : function(str) {
					...
					if (!this.get("effective_readonly")) {
						...
						var $link = this.$el.find('.oe_form_uri')
								.unbind('click').html(link);
						if (!this.options.no_open)
							$link.click(function() {
								var context = self.build_context().eval();
								var model_obj = new instance.web.Model(self.field.relation);
								model_obj.call('get_formview_action',
										[self.get("value"), context]).then(
										function(action) {
				...
				_quick_create : function() {
					this.no_ed = true;
					this.ed_def.reject();
					return instance.web.form.CompletionFieldMixin._quick_create
				...
				_search_create_popup : function() {
					...
					var res = instance.web.form.CompletionFieldMixin._search_create_popup
							.apply(this, arguments);
					this.ignore_focusout = false;
					this.no_ed = false;
					return res;
	...
	instance.web.form.Many2OneButton = instance.web.form.AbstractField.extend({
				...
				set_button : function() {
					...
					this.string = '';
					this.node.attrs.icon = this.get('value')
							? '/web/static/src/img/icons/gtk-yes.png'
							: '/web/static/src/img/icons/gtk-no.png';
				...
				on_click : function(ev) {
					...
					this.popup.show_element(this.field.relation, this
									.get('value'), this.build_context(), {
								title : this.string
							});
	...
	/**
	 * Abstract-ish ListView.List subclass adding an "Add an item" row to
	 * replace the big ugly button in the header.
	 * 
	 * Requires the implementation of a ``is_readonly`` method (usually a proxy
	 * to the corresponding field's readonly or effective_readonly property) to
	 * decide whether the special row should or should not be inserted.
	 * 
	 * Optionally an ``_add_row_class`` attribute can be set for the class(es)
	 * to set on the insertion row.
	 */
	instance.web.form.AddAnItemList = instance.web.ListView.List.extend({
				pad_table_to : function(count) {
					...
					var $cell = $('<td>', {
								...
								e.preventDefault();
								e.stopPropagation();
								// FIXME: there should also be an API for that
								// one
					...
					var $padding = this.$current
							.find('tr:not([data-id]):first');
	...
	/*
	 * # Values: (0, 0, { fields }) create # (1, ID, { fields }) update # (2,
	 * ID) remove (delete) # (3, ID) unlink one (target id or target of
	 * relation) # (4, ID) link # (5) unlink all (only valid for one2many)
	 */
	var commands = {
		// (0, _, {values})
		CREATE : 0,
	...
	instance.web.form.FieldOne2Many = instance.web.form.AbstractField.extend({
		...
		init : function(field_manager, node) {
			...
			this.is_started = false;
			this.dataset = new instance.web.form.One2ManyDataSet(this,
					this.field.relation);
		...
		load_views : function() {
			...
			_.each(modes, function(mode) {
						if (!_.include(["list", "tree", "graph", "kanban"],
								mode)) {
							throw new Error(_.str
									.sprintf(
											_t("View type '%s' is not supported in One2Many."),
											mode));
						}
						var view = {
							view_id : false,
			...
			this.viewmanager = new instance.web.form.One2ManyViewManager(this,
					this.dataset, views, {});
			...
			this.viewmanager.on("controller_inited", self, function(view_type,
					controller) {
				controller.o2m = self;
				if (view_type == "list") {
					if (self.get("effective_readonly")) {
					...
					if (self.get("effective_readonly")) {
						$(".oe_form_buttons", controller.$el).children()
								.remove();
					}
					controller.on("load_record", self, function() {
								once.resolve();
							});
					controller.on('pager_action_executed', self,
							self.save_any_view);
			...
			this.viewmanager.on("switch_mode", self, function(n_mode, b, c, d,
							e) {
		...
		reload_current_view : function() {
			...
			self.is_loaded = self.is_loaded.then(function() {
						...
						if (view.type === "list") {
							...
							if (self.dataset.index === null
									&& self.dataset.ids.length >= 1) {
								self.dataset.index = 0;
							}
							var act = function() {
								return view.controller.do_show();
							};
							self.form_last_update = self.form_last_update.then(
									act, act);
							return self.form_last_update;
						} else if (view.controller.do_search) {
							return view.controller.do_search(self
											.build_domain(), self.dataset
											.get_context(), []);
						}
					}, undefined);
				...
				_.each(value_, function(command) {
							...
							switch (command[0]) {
								case commands.CREATE :
									obj['id'] = _
											.uniqueId(self.dataset.virtual_id_prefix);
									obj.defaults = {};
									self.dataset.to_create.push(obj);
									self.dataset.cache.push(_.extend(_
													.clone(obj), {
		...
		get_value : function() {
			...
			val = val.concat(_.map(this.dataset.ids, function(id) {
						var alter_order = _.detect(self.dataset.to_create,
								function(x) {
									return x.id === id;
								});
						if (alter_order) {
						...
						alter_order = _.detect(self.dataset.to_write, function(
										x) {
									return x.id === id;
								});
						if (alter_order) {
							return commands.update(alter_order.id,
									alter_order.values);
						}
		...
		is_syntax_valid : function() {
			...
			switch (this.viewmanager.active_view) {
				case 'form' :
					return _(view.controller.fields).chain().invoke('is_valid')
	...
	instance.web.form.One2ManyViewManager = instance.web.ViewManager.extend({
				...
				switch_mode : function(mode, unused) {
					...
					var self = this;
					var id = self.o2m.dataset.index !== null
							? self.o2m.dataset.ids[self.o2m.dataset.index]
							: null;
					var pop = new instance.web.form.FormOpenPopup(this);
					pop.show_element(self.o2m.field.relation, id, self.o2m
									.build_context(), {
								title : _t("Open: ") + self.o2m.string,
								create_function : function(data, options) {
									return self.o2m.dataset.create(data,
											options).done(function(r) {
										self.o2m.dataset
												.set_ids(self.o2m.dataset.ids
														.concat([r]));
										self.o2m.dataset.trigger(
												"dataset_changed", r);
									});
								},
								...
								alternative_form_view : self.o2m.field.views
										? self.o2m.field.views["form"]
										: undefined,
								parent_view : self.o2m.view,
								child_name : self.o2m.name,
								read_function : function() {
									return self.o2m.dataset.read_ids.apply(
											self.o2m.dataset, arguments);
								},
								form_view_options : {
									'not_interactible_on_create' : true
								},
	...
	instance.web.form.One2ManyListView = instance.web.ListView.extend({
		...
		init : function(parent, dataset, view_id, options) {
			...
			this.on('save:before cancel:before', this, this
							.proxy('_before_unedit'));

			this.records.bind('add', this.proxy("changed_records")).bind(
					'remove', this.proxy("changed_records"));
			this.on('save:after', this, this.proxy("changed_records"));
		},
		start : function() {
			var ret = this._super();
			this.$el.off('mousedown.handleButtons').on(
					'mousedown.handleButtons',
					'table button, div a.oe_m2o_cm_button',
					this.proxy('_button_down'));
			return ret;
		},
		...
		is_valid : function() {
			...
			var valid = _.every(this.records.records, function(record) {
						_.each(self.editor.form.fields, function(field) {
									field
											.set_value(record.attributes[field.name]);
								});
						return _.every(self.editor.form.fields,
								function(field) {
			...
			if (this.editable()) {
				...
				pop.select_element(self.o2m.field.relation, {
							...
							alternative_form_view : self.o2m.field.views
									...
									: undefined,
							create_function : function(data, options) {
								return self.o2m.dataset.create(data, options)
										.done(function(r) {
											self.o2m.dataset
													.set_ids(self.o2m.dataset.ids
															.concat([r]));
		...
		do_activate_record : function(index, id) {
			...
			pop.show_element(self.o2m.field.relation, id, self.o2m
						...
						write_function : function(id, data) {
							return self.o2m.dataset.write(id, data, {}).done(
									function() {
										self.o2m.reload_current_view();
									});
						},
						...
						readonly : !this.is_action_enabled('edit')
								|| self.o2m.get("effective_readonly")
					});
		},
		do_button_action : function(name, id, callback) {
			if (!_.isNumber(id)) {
				instance.webclient.notification
						.warn(
								_t("Action Button"),
			...
			this.ensure_saved().then(function() {
				...
				var cached_records = _.any([ds.to_create, ds.to_delete,
								ds.to_write], function(value) {
							return value.length;
						});
		...
		keypress_ENTER : function() {
			...
			// (save
			// the current row *anyway*, then create a new one/edit the next
			// one)
			this.__ignore_blur = true;
			this._super.apply(this, arguments);
		},
		...
		reload_record : function(record) {
			// Evict record.id from cache to ensure it will be reloaded
			// correctly
	...
	instance.web.form.One2ManyFormView = instance.web.FormView.extend({
				...
				load_form : function(data) {
					this._super(data);
					var self = this;
					this.$buttons.find('button.oe_form_button_create').click(
							function() {
	...
	var lazy_build_o2m_kanban_view = function() {
		if (!instance.web_kanban || instance.web.form.One2ManyKanbanView)
			return;
		instance.web.form.One2ManyKanbanView = instance.web_kanban.KanbanView
				.extend({});
	};

	instance.web.form.FieldMany2ManyTags = instance.web.form.AbstractField
			.extend(instance.web.form.CompletionFieldMixin,
					instance.web.form.ReinitializeFieldMixin, {
						template : "FieldMany2ManyTags",
						tag_template : "FieldMany2ManyTag",
						init : function() {
							this._super.apply(this, arguments);
							instance.web.form.CompletionFieldMixin.init
									.call(this);
							this.set({
										"value" : []
									});
						...
						initialize_content : function() {
							...
							self.$text.textext(self.initialize_texttext())
									.bind('getSuggestions', function(e, data) {
										...
										var str = !!data
												? data.query || ''
												: '';
										self.get_search_result(str).done(
												function(result) {
							...
							self.$text.focusin(function() {
								...
								if (e.which === $.ui.keyCode.TAB
										&& self._drop_shown) {
									self.$text.textext()[0].autocomplete()
											.selectFromDropdown();
								}
							});
						},
						...
						set_value : function(value_) {
							value_ = value_ || [];
							if (value_.length >= 1
									&& value_[0] instanceof Array) {
								// value_ is a list of m2m commands. We only
								// process
						...
						render_tag : function(data) {
							var self = this;
							if (!self.get("effective_readonly")) {
								self.tags.containerElement().children()
										.remove();
								self.$('textarea').css("padding-left", "3px");
								self.tags.addTags(self.map_tag(data));
							} else {
								self.$el.html(QWeb.render(self.tag_template, {
											elements : data
										}));
								// liuhongtai 2015-12-02
								// 增加，处理ie下many2many_tags显示
								self.$(".oe_form_field oe_tags").css(
						...
						render_value : function() {
							...
							if (!values || values.length > 0) {
								return this._display_orderer.add(self
										.get_render_data(values))
										.done(handle_names);
							} else {
								handle_names([]);
							}
						...
						_search_create_popup : function() {
							self.ignore_blur = true;
							return instance.web.form.CompletionFieldMixin._search_create_popup
									.apply(this, arguments);
						}
						,
					});
	...
	/**
	 * widget options: - reload_on_button: Reload the whole form view if click
	 * on a button in a list view. If you see this options, do not use it, it's
	 * basically a dirty hack to make one precise o2m to behave the way we want.
	 */
	instance.web.form.FieldMany2Many = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				...
				init : function(field_manager, node) {
					this._super(field_manager, node);
					this.is_loaded = $.Deferred();
					this.dataset = new instance.web.form.Many2ManyDataSet(this,
							this.field.relation);
				...
				initialize_content : function() {
					...
					this.list_view = new instance.web.form.Many2ManyListView(
							this, this.dataset, false, {
								'addable' : null,
								'deletable' : this.get("effective_readonly")
										? false
										: true,
	...
	instance.web.form.Many2ManyListView = instance.web.ListView.extend(/** @lends instance.web.form.Many2ManyListView# */
		...
		do_add_record : function() {
			...
			pop.select_element(this.model, {
						...
						,
					}, new instance.web.CompoundDomain(this.m2m_field
									.build_domain(), ["!",
									["id", "in", this.m2m_field.dataset.ids]]),
			...
			pop.on("elements_selected", self, function(element_ids) {
						...
						_(element_ids).each(function(id) {
							if (!_.detect(self.dataset.ids, function(x) {
								...
								self.dataset.set_ids(self.dataset.ids
										.concat([id]));
		...
		do_activate_record : function(index, id) {
			var self = this;
			var pop = new instance.web.form.FormOpenPopup(this);
			pop.show_element(this.dataset.model, id, this.m2m_field
							.build_context(), {
	...
	instance.web.form.FieldMany2ManyKanban = instance.web.form.AbstractField
			.extend(instance.web.form.CompletionFieldMixin, {
				disable_utility_classes : true,
				init : function(field_manager, node) {
					this._super(field_manager, node);
				...
				start : function() {
					...
					self.on("change:effective_readonly", self, function() {
								self.is_loaded = self.is_loaded.then(
										function() {
											self.kanban_view.destroy();
											return $.when(self.load_view())
													.done(function() {
																self
																		.render_value();
															});
				...
				load_view : function() {
					var self = this;
					this.kanban_view = new instance.web.form.Many2ManyKanbanView(
							this, this.dataset, false, {
								'create_text' : _t("Add"),
								'creatable' : self.get("effective_readonly")
										? false
										: true,
								'quick_creatable' : self
										.get("effective_readonly")
										? false
										: true,
								'read_only_mode' : self
				...
				render_value : function() {
					...
					this.is_loaded = this.is_loaded.then(function() {
								return self.kanban_view.do_search(self
												.build_domain(), self.dataset
												.get_context(), []);
							});
				},
				...
				open_popup : function(type, unused) {
					...
					if (this.dataset.index === null) {
										...
										new instance.web.CompoundDomain(
												...
												[
														...
														[
																...
																this.dataset.ids]]),
										this.build_context());
						pop.on("elements_selected", self,
								function(element_ids) {
									_.each(element_ids, function(one_id) {
												if (!_.detect(self.dataset.ids,
														function(x) {
															return x == one_id;
														})) {
													self.dataset
						...
						pop.show_element(self.field.relation, id, self
										.build_context(), {
									title : _t("Open: ") + self.string,
									write_function : function(id, data, options) {
										return self.dataset.write(id, data, {})
												.done(function() {
															self.render_value();
														});
									},
									alternative_form_view : self.field.views
											? self.field.views["form"]
											: undefined,
	...
	function m2m_kanban_lazy_init() {
		if (instance.web.form.Many2ManyKanbanView)
			return;
		instance.web.form.Many2ManyKanbanView = instance.web_kanban.KanbanView
				.extend({
		...
		instance.web.form.Many2ManyQuickCreate = instance.web.Widget.extend({
					...
					/**
					 * close_btn: If true, the widget will display a "Close"
					 * button able to trigger a "close" event.
					 */
					...
					start : function() {
						var self = this;
						self.$text = this.$el.find('input').css("width",
								"200px");
						self.$text.textext({
							...
							self.m2m.get_search_result(str).done(
									function(result) {
										self.search_result = result;
	...
	/**
	 * Class with everything which is common between FormOpenPopup and
	 * SelectCreatePopup.
	 */
	instance.web.form.AbstractFormPopup = instance.web.Widget.extend({
		template : "AbstractFormPopup.render",
		/**
		 * options: -readonly: only applicable when not in creation mode,
		 * default to false - alternative_form_view - view_id - write_function -
		 * read_function - create_function - parent_view - child_name -
		 * form_view_options
		 */
		...
		setup_form_view : function() {
			...
			this.view_form = new instance.web.FormView(this, this.dataset,
					this.options.view_id || false, options);
			if (this.options.alternative_form_view) {
				this.view_form
						.set_embedded_view(this.options.alternative_form_view);
			}
			this.view_form.appendTo(this.$el.find(".oe_popup_form"));
			this.view_form.on("form_view_loaded", self, function() {
						var multi_select = self.row_id === null
								&& !self.options.disable_multiple_selection;
						self.$buttonpane.html(QWeb.render(
								"AbstractFormPopup.buttons", {
									multi_select : multi_select,
									readonly : self.row_id !== null
											&& self.options.readonly
									,
								}));
						var $snbutton = self.$buttonpane
								.find(".oe_abstractformpopup-form-save-new");
						$snbutton.click(function() {
									$.when(self.view_form.save()).done(
											function() {
												self.view_form.reload_mutex
														.exec(function() {
															self.view_form
																	.on_button_new();
														});
						...
						var $sbutton = self.$buttonpane
								.find(".oe_abstractformpopup-form-save");
						...
						var $cbutton = self.$buttonpane
								.find(".oe_abstractformpopup-form-close");
	 ...
	 */
	instance.web.form.FormOpenPopup = instance.web.form.AbstractFormPopup
			.extend({
						show_element : function(model, row_id, context, options) {
							this
									.init_popup(model, row_id, [], context,
											options);
							_.defaults(this.options, {});
							this.display_popup();
						},
	...
	/**
	 * Class to display a popup to display a list to search a row. It also
	 * allows to switch to a form view to create a new row.
	 */
	instance.web.form.SelectCreatePopup = instance.web.form.AbstractFormPopup
			.extend({
				/**
				 * options: - initial_ids - initial_view: form or search
				 * (default search) - disable_multiple_selection -
				 * list_view_options
				 */
				...
				setup_search_view : function(search_defaults) {
					...
					this.searchview_drawer = new instance.web.SearchViewDrawer(
							this, this.searchview);
					this.searchview.on('search_data', self, function(domains,
									contexts, groupbys) {
								if (self.initial_ids) {
									self.do_search(domains.concat([
													[["id", "in",
															self.initial_ids]],
													self.domain]), contexts
													.concat(self.context),
											groupbys);
									self.initial_ids = undefined;
								} else {
									self.do_search(domains
													.concat([self.domain]),
											contexts.concat(self.context),
											groupbys);
								}
							});
					this.searchview.on("search_view_loaded", self, function() {
						self.view_list = new instance.web.form.SelectCreateListView(
								self, self.dataset, false, _.extend({
									'deletable' : false,
						...
						self.view_list.appendTo($(".oe_popup_list", self.$el))
								.then(function() {
						...
						self.view_list.on("list_view_loaded", self, function() {
							self.$buttonpane.html(QWeb.render(
									"SelectCreatePopup.search.buttons", {
										widget : self
									}));
							var $cbutton = self.$buttonpane
									.find(".oe_selectcreatepopup-search-close");
							$cbutton.click(function() {
										self.destroy();
									});
							var $sbutton = self.$buttonpane
									.find(".oe_selectcreatepopup-search-select");
							$sbutton.click(function() {
										self.select_elements(self.selected_ids);
										self.destroy();
									});
							$cbutton = self.$buttonpane
									.find(".oe_selectcreatepopup-search-create");
							$cbutton.click(function() {
										self.new_object();
									});
						});
				...
				do_search : function(domains, contexts, groupbys) {
					...
					instance.web.pyeval.eval_domains_and_contexts({
						...
						self.view_list.do_search(results.domain,
								results.context, results.group_by);
					});
				},
	...
	instance.web.form.FieldReference = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				...
				initialize_content : function() {
					...
					fm.extend_field_desc({
								"selection" : {
									selection : this.field_manager
											.get_field_desc(this.name).selection,
									type : "selection"
									,
								},
								"m2o" : {
					...
					this.selection.on("change:value", this,
							this.on_selection_changed);
					this.selection.appendTo(this
							.$(".oe_form_view_reference_selection"));
				...
				on_selection_changed : function() {
					if (this.reference_ready) {
						this.internal_set_value([this.selection.get_value(),
								false]);
						this.render_value();
					}
				},
				...
				get_value : function() {
					return this.get('value')[0] && this.get('value')[1] ? (this
							.get('value')[0]
							+ ',' + this.get('value')[1]) : false;
				},
				render_value : function() {
	...
	instance.web.form.FieldBinary = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				init : function(field_manager, node) {
					var self = this;
					this._super(field_manager, node);
				...
				initialize_content : function() {
					var self = this;
					this.$el.find('input.oe_form_binary_file')
							.change(this.on_file_change);
					this.$el.find('button.oe_form_binary_file_save')
							.click(this.on_save_as);
					this.$el.find('.oe_form_binary_file_clear')
							.click(this.on_clear);
					this.$el.find('.oe_form_binary_file_edit').click(
							function(event) {
								self.$el.find('input.oe_form_binary_file')
										.click();
							});
				},
					...
					if ((this.useFileAPI && file_node.files.length)
						...
						if (this.useFileAPI) {
										...
										.warn(
												...
												_.str
														.sprintf(
																msg,
																instance.web
																		.human_size(this.max_upload_size)));
								return false;
							}
							...
							filereader.onloadend = function(upload) {
								...
								data = data.split(',')[1];
								self.on_file_uploaded(file.size, file.name,
										file.type, data);
							};
						} else {
							this.$el
									.find('form.oe_form_binary_form input[name=session_id]')
									.val(this.session.session_id);
				...
				on_save_as : function(ev) {
					...
					if (!value) {
						...
						var filename_field = this.view.fields
								&& this.view.fields[filename_fieldname];
						this.session.get_file({
							...
							data : {
								data : JSON.stringify({
									...
									data : instance.web.form.is_bin_size(value)
											? null
											: value,
									filename : filename_field ? filename_field
											.get('value') : null,
		...
		render_value : function() {
			...
			if (!this.get("effective_readonly")) {
				if (this.node.attrs.filename) {
					...
					show_value = (this.get('value') !== null
							&& this.get('value') !== undefined && this
							.get('value') !== false) ? this.get('value') : '';
				}
				...
				if (this.get('value')) {
					...
					if (this.view)
						show_value += " "
								+ (this.view.datarecord[this.node.attrs.filename] || '');
	...
	instance.web.form.FieldBinaryImage = instance.web.form.FieldBinary.extend({
		...
		render_value : function() {
			...
			$img.load(function() {
						...
						$img
								.css("max-height", "" + self.options.size[1]
												+ "px");
					});
		...
		set_value : function(value_) {
			...
			this._super.apply(this, arguments);
			// By default, on binary images read, the server returns the binary
			// size
			// This is possible that two images have the exact same size
			// Therefore we trigger the change in case the image value hasn't
			// changed
	 ...
	 * display in list. The user can delete his files. Options on attribute ;
	 * "blockui" {Boolean} block the UI or not during the file is uploading
	 */
	instance.web.form.FieldMany2ManyBinaryMultiFiles = instance.web.form.AbstractField
			.extend(instance.web.form.ReinitializeFieldMixin, {
				...
				init : function(field_manager, node) {
					...
					if (this.field.type != "many2many"
							|| this.field.relation != 'ir.attachment') {
						throw _.str
								.sprintf(
										_t("The type of the field '%s' must be a many2many field with a relation to 'ir.attachment' model."),
										this.field.string);
					}
					this.data = {};
					this.set_value([]);
					this.ds_file = new instance.web.DataSetSearch(this,
							'ir.attachment');
					this.fileupload_id = _.uniqueId('oe_fileupload_temp');
					$(window).on(this.fileupload_id,
							_.bind(this.on_file_loaded, this));
				},
				initialize_content : function() {
					this.$el.on('change', 'input.oe_form_binary_file',
							this.on_file_change);
				},
				// WARNING: duplicated in 4 other M2M widgets
				set_value : function(value_) {
				...
				read_name_values : function() {
					...
					var _value = _.filter(ids, function(id) {
								return typeof self.data[id] == 'undefined';
							});
					// send request for get_name
					if (_value.length) {
						return this.ds_file.call('read',
								[_value, ['id', 'name', 'datas_fname']]).then(
								function(datas) {
									_.each(datas, function(data) {
										data.no_unlink = true;
										data.url = self.session.url(
												'/web/binary/saveas', {
													model : 'ir.attachment',
													field : 'datas',
													filename_field : 'datas_fname',
				...
				render_value : function() {
					...
					this.read_name_values().then(function(ids) {
						var render = $(instance.web.qweb.render(
								'FieldBinaryFileUploader.files', {
									'widget' : self,
									'values' : ids
								}));
						render.on('click', '.oe_delete', _.bind(
										self.on_file_delete, self));
				...
				on_file_change : function(event) {
					...
					if ($target.val() !== '') {
						...
						for (var id in this.get('value')) {
							...
							// (if it's a new file)
							if (self.data[id]
									&& (self.data[id].filename || self.data[id].name) == filename
				...
				on_file_loaded : function(event, result) {
					...
					if (result.error || !result.id) {
						...
						if (this.data[0]
								&& this.data[0].filename == result.filename
								&& this.data[0].upload) {
							delete this.data[0];
							this.data[result.id] = {
								'id' : result.id,
	...
	instance.web.form.FieldStatus = instance.web.form.AbstractField.extend({
		...
		init : function(field_manager, node) {
			this._super(field_manager, node);
			this.options.clickable = this.options.clickable
					|| (this.node.attrs || {}).clickable || false;
			this.options.visible = this.options.visible
					|| (this.node.attrs || {}).statusbar_visible || false;
			this.set({
						value : false
					});
			this.selection = {
			...
			this.dataset = new instance.web.DataSetStatic(this,
					this.field.relation, this.build_context());
		},
		start : function() {
			this.field_manager.on("view_content_has_changed", this,
					this.calc_domain);
			...
			this.on("change:evaluated_selection_domain", this,
					this.get_selection);
		...
		render_value : function() {
			...
			var content = QWeb.render("FieldStatus.content", {
						'widget' : self,
						'value_folded' : _.find(self.selection.folded,
								function(i) {
									return i[0] === self.get('value');
								})
					});
			// 20151227，李勇，新增，解决多条分支情况下仅改变最后一个状态条样式的问题
			var siblings = self.$el.siblings('ul');
			if (siblings.length >= 1) {
				siblings.children('li[data-id=' + self.get_value() + ']')
						.addClass('oe_active').siblings()
						.removeClass('oe_active');
				self.$el.siblings('div').remove();
				self.$el.parent().append('<div class="oe_clear"></div>');
			}
			// 新增结束
			self.$el.html(content);
			// 同步隐藏不可见状态条的clear div，消除换行的效果
			/*
			 * if(self.$el.hasClass('oe_form_invisible')){
			 * self.$el.next('div').hide(); }else{ self.$el.next('div').show(); }
			 * if(self.$el.siblings('ul').length>0){ for(var i=0;i<siblings.length;i++){
			 * if($(siblings[i]).hasClass('oe_form_invisible')){
			 * $(siblings[i]).next('div').hide(); }else{
			 * $(siblings[i]).next('div').show(); } } }
			 */
		},
		calc_domain : function() {
			var d = instance.web.pyeval.eval('domain', this.build_domain());
			var domain = []; // if there is no domain defined, fetch all the
			// records
		...
		/**
		 * Get the selection and render it selection: [[identifier,
		 * value_to_display], ...] For selection fields: this is directly given
		 * by this.field.selection For many2one fields: perform a search on the
		 * relation of the many2one field
		 */
		get_selection : function() {
			var self = this;
			...
			var calculation = _.bind(function() {
				if (this.field.type == "many2one") {
					return self.get_distant_fields().then(function(fields) {
						return new instance.web.DataSetSearch(self,
								self.field.relation, self.build_context(), self
										.get("evaluated_selection_domain"))
								.read_slice(
										_.union(_.keys(self.distant_fields),
												['id']), {}).then(
										function(records) {
					...
					for (var i = 0; i < select.length; i++) {
						var key = select[i][0];
						if (key == this.get('value') || !this.options.visible
								|| this.options.visible.indexOf(key) != -1) {
			...
			this.selection_dm.add(calculation()).then(function() {
						var selection = {
							'unfolded' : selection_unfolded,
							'folded' : selection_folded
						};
		...
		get_distant_fields : function() {
			...
			return new instance.web.Model(self.field.relation).call(
					"fields_get", [[this.options.fold_field]]).then(
					function(fields) {
		...
		on_click_stage : function(ev) {
			...
			if (val != self.get('value')) {
				this.view.recursive_save().done(function() {
					...
					self.view.dataset.write(self.view.datarecord.id, change)
							.done(function() {
										self.view.reload();
									});
				});
	...
	instance.web.form.FieldMonetary = instance.web.form.FieldFloat.extend({
		...
		init : function() {
			...
			if (this.options.currency_field) {
				this.field_manager.on("field_changed:"
								+ this.options.currency_field, this,
						function() {
							this.set({
								"currency" : this.field_manager
		...
		get_currency_info : function() {
			...
			return this.ci_dm.add(self
					.alive(new instance.web.Model("res.currency").query([
							"symbol", "position"]).filter([["id", "=",
							self.get("currency")]]).first())).then(
					function(res) {
						self.set({
									"currency_info" : res
								});
	...
	/*
	 * This type of field display a list of checkboxes. It works only with m2ms.
	 * This field will display one checkbox for each record existing in the
	 * model targeted by the relation, according to the given domain if one is
	 * specified. Checked records will be added to the relation.
	 */
	instance.web.form.FieldMany2ManyCheckBoxes = instance.web.form.AbstractField
			.extend(instance.web.form.ReinitializeFieldMixin, {
				...
				query_records : function() {
					...
					var model = new openerp.Model(openerp.session,
							this.field.relation);
					this.records_orderer.add(model.call("search",
							[this.get("domain")], {
								"context" : this.build_context()
							}).then(function(record_ids) {
								return model.call("name_get", [record_ids], {
				...
				render_value : function() {
					this.$().html(QWeb.render("FieldMany2ManyCheckBoxes", {
								widget : this,
				...
				from_dom : function() {
					...
					this.$("input").each(function() {
						var elem = $(this);
						new_value[elem.data("record-id")] = elem
								.attr("checked") ? true : undefined;
					});
				...
				get_value : function() {
					var value = _.filter(_.keys(this.get("value")),
							function(el) {
	...
	/**
	 * This field can be applied on many2many and one2many. It is a read-only
	 * field that will display a single link whose name is "<number of linked
	 * records> <label of the field>". When the link is clicked, it will
	 * redirect to another act_window action on the model of the relation and
	 * show only the linked records.
	 * 
	 * Widget options:
	 * 
	 * views: The views to display in the act_window action. Must be a list of
	 * tuples whose first element is the id of the view to display (or False to
	 * take the default one) and the second element is the type of the view.
	 * Defaults to [[false, "tree"], [false, "form"]] .
	 */
	instance.web.form.X2ManyCounter = instance.web.form.AbstractField.extend(
			instance.web.form.ReinitializeFieldMixin, {
				...
				render_value : function() {
					var text = _.str.sprintf("%d %s", this.val().length,
							this.string);
					this.$().html(QWeb.render("X2ManyCounter", {
								text : text
							}));
	...
	/**
	 * This widget is intended to be used on stat button numeric fields. It will
	 * display the value many2many and one2many. It is a read-only field that
	 * will display a simple string "<value of field> <label of the field>"
	 */
	...
	instance.web.form.CustomTable = instance.web.form.AbstractField.extend({
				multi_selection : false,
				disable_utility_classes : true,
				init : function(field_manager, node) {
					this._super(field_manager, node);
	 ...
	 * 
	 * All referenced classes must implement FieldInterface. Those represent the
	 * classes whose instances will substitute to the <field> tags as defined in
	 * OpenERP's views.
	 */
	instance.web.form.widgets = new instance.web.Registry({
		...
		'c_table' : 'instance.web.form.CustomTable',
		'attachments' : 'instance.web.form.Attachments'
	});

	/**
	 * Registry of widgets usable in the form view that can substitute to any
	 * possible tags defined in OpenERP's form views.
	 * 
