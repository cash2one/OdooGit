...
class ir_attachment(osv.osv):
    ...
    _columns = {
        ...
        #added by wangkui 2016-04-07
        #文件类型（视频，图片...）
        'category': fields.char('File Category'),
        #手机端提供的媒体文件id，字符串形式
        'media_id': fields.char('Media ID'),
        #在音频附件中存储音频时间长度，其他附件存空
        'length': fields.char('Media Length'),
    }
    ...
    @api.model
    def get_attachments(self,ids,res_model,res_id):
        """
        获取模型单条记录的所有附件，或根据给定的ids查询指定的附件
        """
        
        if ids and len(ids) > 0:
            ids = str(ids).replace('[', '(').replace(']', ')')
            sql='select * from ir_attachment where id in ' + ids
        else:
            sql='select * from ir_attachment where res_model = \'' + res_model +'\' and res_id = ' + str(res_id)
        try:
            query = self.env.cr
            query.execute(sql)
            result = query.dictfetchall()
        except:
            result=''
        return result

    def get_attachment(self, cr, uid, attachment_id, context=None):
        """ 
        获取单个附件的信息，用于下载附件
        """
        
        attachment = self.browse(cr, SUPERUSER_ID, attachment_id, context=context)
