...
class ir_ui_menu(osv.osv):
    ...
    def get_user_roots(self, cr, uid, context=None):
        ...
        :rtype: list(int)
        """
        menu_domain = [('parent_id', '=', False),('invisible', '=', False)]
    ...
    def load_menus(self, cr, uid, context=None):
        ...
        """
        fields = ['name', 'sequence', 'parent_id', 'action','web_icon_data','web_icon_hover_data','icon']
        menu_root_ids = self.get_user_roots(cr, uid, context=context)
        menu_roots = self.read(cr, uid, menu_root_ids, fields, context=context) if menu_root_ids else []
        menu_root = {
            'id': False,
        ...
        menu_ids = self.search(cr, uid, [('id', 'child_of', menu_root_ids),('invisible', '=', False)], 0, False, False, context=context)
    ...
    _columns = {
        ...
        'invisible':fields.boolean(u'隐藏菜单',default=False),
