...
class res_users(osv.osv):
    ...
    def _get_staff_id(self):
        for record in self:  
            record.staff_id = self.env['oa.staff.basic'].search([('related_user','=',record.id)]).id
            record.leader_staff_id = self.env['oa.staff.basic'].search([('related_user','=',record.id),('administrative_post_id.name','in',(u'书记',u'中心主任',u'中心副主任',u'所长',u'副所长',u'所长助理'))]).id
    #获取关联的expert_id
    def _get_expert_id(self):
        for record in self:
            record.expert_id = self.env['audit.expert.info'].search([('related_user', '=', record.id)]).id            
    _columns = {
        ...
        'site_id': fields.many2one('audit.vld.site', string='所属组织机构'),
    }
    ...
    #add by liuhongtai 20160127
    staff_id = openerp.fields.Many2one('oa.staff.basic', compute='_get_staff_id', string="员工id")
    leader_staff_id = openerp.fields.Many2one('oa.staff.basic', compute='_get_staff_id', string="所(中心)领导id")
    
    #add by hanlu 20160329
    expert_id = openerp.fields.Many2one('audit.expert.info', compute = '_get_expert_id', string="专家ID")
    ...
    def _login(self, db, login, password):
        ...
        try:
            ...
            res = self.search(cr, SUPERUSER_ID, ['|',('login','=',login),('mobile','=',login)])
            if res:
                user_id = res[0]
                # check credentials
